...
public class FusionShrineBlockEntityRenderer<T extends FusionShrineBlockEntity> implements BlockEntityRenderer<T> {
    ...
    @Override
    public void render(
        FusionShrineBlockEntity shrine, float tickDelta, PoseStack poseStack, MultiBufferSource vertexConsumerProvider,
        int light, int overlay
    ) {
        // the fluid in the shrine
        var fluid = shrine.getTank()
                          .getFluid();
        if (!fluid.isEmpty()) {
            poseStack.pushPose();
            ...
            int[] colors = FluidRendering.unpackColor(renderData.getTintColor(
                fluid.getFluid()
                     .defaultFluidState(), shrine.getLevel(), shrine.getBlockPos()
            ));
            FluidRendering.renderFluid(
                vertexConsumerProvider.getBuffer(RenderType.translucent()), poseStack.last()
                                                                                     .pose(), sprite, light, overlay, 2,
                14, 0.9F, 2, 14, colors
            );
            poseStack.popPose();
        }
        ...
        if (!shrine.isEmpty()) {
            ...
            float time = shrine.getLevel()
                               .getGameTime() % 500000 + tickDelta;
            ...
            for (int i = 0; i < inventoryStacks.size(); i++) {
                ...
                poseStack.translate(
                    distance * Math.sin(currentRadiant) + 0.5, 1.5 + height,
                ...
                Minecraft.getInstance()
                         ...
                         .renderStatic(
                             ...
                             vertexConsumerProvider, shrine.getLevel(), 0
                         );
                poseStack.popPose();
            }
        }
