...
public class BookGatedRecipePage<T extends GatedRecipe<?>> extends BookRecipePage<T> implements GatedGuidebookPage {
    ...
    public BookGatedRecipePage(
        ...
        BookTextHolder title1,
        ResourceLocation recipeId1,
        BookTextHolder title2,
        ResourceLocation recipeId2,
        BookTextHolder text,
        String anchor,
    ...
    public static BookCondition getConditionWithRecipes(
        ...
        BookCondition[] conditions = {
            condition, new RecipesLoadedAndUnlockedCondition(null, list)
        };
    ...
    public static <T extends GatedRecipe<?>> BookGatedRecipePage<T> fromJson(
        ...
        if (supportsTwoRecipesOnOnePage) {
            ...
            ResourceLocation recipeId1 = json.has("recipe_id")
                ? ResourceLocation
                    .tryParse(
                        GsonHelper.getAsString(json, "recipe_id")
                    )
                : null;
            ResourceLocation recipeId2 = json.has("recipe_id2")
                ? ResourceLocation
                    .tryParse(
                        GsonHelper.getAsString(json, "recipe_id2")
                    )
                : null;
