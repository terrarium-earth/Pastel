...
public class TuningStampItem extends Item implements TooltipExtensions {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (potentialData.isPresent()) {
            ...
            if (player
                .map(Entity::isShiftKeyDown)
                .orElse(false)) {
                if (candidate
                    .map(d -> d.canUserStamp(player))
                    .orElse(false)) {
                    candidate
                        .get()
                        .source()
            ...
            if (candidate.isPresent() && candidate
                .get()
                .canUserStamp(player)) {
    ...
    public void appendTooltipWithPlayer(
        ...
        if (player != null && nbtComp.contains(DATA)) {
            ...
            if (data.isEmpty()) {
                tooltip
                    .add(
                        Component
                            .translatable("item.pastel.tuning_stamp.tooltip.missing")
            ...
            tooltip
                .add(
                    Component
                        .translatable(
                            "item.pastel.tuning_stamp.tooltip.linked",
                            stampData
                                .reference()
                                .getState()
                                .getBlock()
            ...
            tooltip
                .add(
                    Component
                        .translatable("item.pastel.tuning_stamp.tooltip2", pos.getX(), pos.getY(), pos.getZ())
                        .withStyle(
                            style -> style
                                .withColor(0xf99b89)
                                .withItalic(true)
        ...
        tooltip
            .add(
                Component
                    .translatable("item.pastel.tuning_stamp.tooltip")
    ...
    public void expandTooltipPostStats(
        ...
        if (Screen.hasShiftDown()) {
            tooltip
                .add(
                    Component
                        .translatable("item.pastel.tuning_stamp.controls")
                        .withStyle(style -> style.withColor(0x66ff99))
                );
            tooltip
                .add(
                    Component
                        .translatable("item.pastel.tuning_stamp.controls2")
                        .withStyle(style -> style.withColor(0x66ff99))
                );
            tooltip
                .add(
                    Component
                        .translatable("item.pastel.tuning_stamp.controls3")
                        .withStyle(style -> style.withColor(0x66ff99))
                );
        } else {
            tooltip
                .add(
                    Component
                        .translatable("pastel.tooltip.press_shift_for_controls")
