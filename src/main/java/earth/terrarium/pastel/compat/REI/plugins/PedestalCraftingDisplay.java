...
public class PedestalCraftingDisplay extends PastelDisplay {
    ...
    public PedestalCraftingDisplay(RecipeHolder<PedestalRecipe> recipe) {
        super(recipe, mapIngredients(recipe.value()), Collections.singletonList(EntryIngredients.of(recipe.value()
                                                                                                          .getResultItem(
                                                                                                              BasicDisplay.registryAccess())))
        );
        this.pedestalRecipeTier = recipe.value()
                                        .getTier();
        this.width = recipe.value()
                           .getWidth();
        this.height = recipe.value()
                            .getHeight();
        this.experience = recipe.value()
                                .getExperience();
        this.craftingTime = recipe.value()
                                  .getCraftingTime();
        this.shapeless = recipe.value()
                               .isShapeless();
    }
    ...
    private static List<EntryIngredient> mapIngredients(PedestalRecipe recipe) {
        ...
        for (int i = 0; i < ingredientCount; i++) {
            list.set(recipe.getGridSlotId(i), REIHelper.ofIngredientStack(recipe.getIngredientStacks()
                                                                                .get(i))
            );
        }
