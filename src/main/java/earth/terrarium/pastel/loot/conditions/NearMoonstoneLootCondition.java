...
public record NearMoonstoneLootCondition(int searchRadius) implements LootItemCondition {
    public static final MapCodec<NearMoonstoneLootCondition> CODEC = RecordCodecBuilder
        .mapCodec(
            i -> i
                .group(
                    Codec.INT
                        .fieldOf("search_radius")
    ...
    public boolean test(LootContext context) {
        ...
        var searchArea = BlockPos
            .withinManhattan(
                new BlockPos(Mth.floor(origin.x()), Mth.floor(origin.y()), Mth.floor(origin.z())),
                searchRadius,
                searchRadius,
                searchRadius
            );
        for (
            BlockPos i : searchArea
        ) {
            if (context
                .getLevel()
                .getBlockState(i)
