...
public class PedestalCraftingCategory extends GatedDisplayCategory<PedestalCraftingDisplay> {
    ...
    public void setupWidgets(
        Point startPoint, Rectangle bounds, List<Widget> widgets, @NotNull PedestalCraftingDisplay display) {
        ...
        int powderSlotCount = display.getTier()
                                     .getPowderSlotCount();
        ...
        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 3; y++) {
                slots.add(Widgets.createSlot(new Point(startPoint.x + 1 + x * 18, startPoint.y + 2 + y * 18))
                                 .disableBackground()
                                 .markInput()
                                 .disableBackground()
                                 .entries(input.get(y * 3 + x)));
            }
        }

        // powder slots
        for (int i = 0; i < powderSlotCount; i++) {
            slots.add(Widgets.createSlot(new Point(gemstoneSlotStartX + 1 + i * 18, startPoint.y + 61))
                             .disableBackground()
                             .markInput()
                             .entries(input.get(gemstoneDustStartSlot + i)));
        }
        widgets.addAll(slots);

        // the gemstone slot background texture				  destinationX				 destinationY	   sourceX,
        // sourceY, width, height
        widgets.add(
            Widgets.createTexturedWidget(
                backgroundTexture, gemstoneSlotStartX, startPoint.y + 60, gemstoneSlotTextureU, 76,
                powderSlotCount * 18, 18
            ));
        ...
        widgets.add(
            Widgets.createTexturedWidget(backgroundTexture, startPoint.x + 90, startPoint.y + 15, 122, 32, 26, 26));
        // miniature gemstones texture
        widgets.add(
            Widgets.createTexturedWidget(backgroundTexture, startPoint.x + 82, startPoint.y + 39, 200, 0, 40, 16));

        // description text
        Component text = getCraftingTimeText(display.craftingTime, display.experience);
        widgets.add(Widgets.createLabel(new Point(bounds.getCenterX(), startPoint.y + 82), text)
                           .centered()
                           .color(0x3f3f3f)
                           .noShadow());
        ...
        widgets.add(Widgets.createSlot(new Point(startPoint.x + 95, startPoint.y + 20))
                           .entries(result)
                           .disableBackground()
                           .markOutput());
    }
