...
public class CraftingTabletScreenHandler extends RecipeBookMenu<RecipeInput, Recipe<RecipeInput>> {
    ...
    public CraftingTabletScreenHandler(
        int syncId, Inventory playerInventory, ContainerLevelAccess context, ItemStack craftingTabletItemStack) {
        ...
        // preview slot
        lockableCraftingResultSlot = new LockableCraftingResultSlot(
            craftingResultInventory, 0, 127, 37, playerInventory.player, craftingInventory);
    ...
    protected void updateResult(
        AbstractContainerMenu handler, @NotNull Level world, Player player, CraftingTabletInventory inventory) {
        if (!world.isClientSide) {
            ServerPlayer serverPlayerEntity = (ServerPlayer) player;

            PedestalRecipeInput pedestalRecipeInput = PedestalRecipeInput.createWithFullGemstonePowder(
                inventory.getItems(), player);

            Optional<RecipeHolder<PedestalRecipe>> optionalPedestalCraftingRecipe = world.getRecipeManager()
                                                                                         .getRecipeFor(
                                                                                             PastelRecipeTypes.PEDESTAL,
                                                                                             pedestalRecipeInput, world
                                                                                         );
            if (optionalPedestalCraftingRecipe.isPresent()) {
                ...
                PedestalRecipe pedestalRecipe = optionalPedestalCraftingRecipe.get()
                                                                              .value();
                ItemStack itemStack = pedestalRecipe.getResultItem(world.registryAccess())
                                                    .copy();
                ...
                serverPlayerEntity.connection.send(
                    new ClientboundContainerSetSlotPacket(
                        handler.containerId, handler.incrementStateId(), 14,
                        itemStack
                    ));
                ...
                Optional<RecipeHolder<CraftingRecipe>> optionalCraftingRecipe = world.getRecipeManager()
                                                                                     .getRecipeFor(
                                                                                         RecipeType.CRAFTING,
                                                                                         pedestalRecipeInput.getCraftingGridInput(),
                                                                                         world
                                                                                     );
                if (optionalCraftingRecipe.isPresent()) {
                    lockableCraftingResultSlot.unlock();

                    if (craftingResultInventory.setRecipeUsed(
                        world, serverPlayerEntity, optionalCraftingRecipe.get())) {
                        itemStack = optionalCraftingRecipe.get()
                                                          .value()
                                                          .assemble(
                                                              pedestalRecipeInput.getCraftingGridInput(),
    ...
    public boolean recipeMatches(RecipeHolder recipe) {
        PedestalRecipeInput pedestalRecipeInput = PedestalRecipeInput.createWithFullGemstonePowder(
            this.craftingInventory.getItems(), player);
    ...
    public void removed(Player playerEntity) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (!itemStack.isEmpty()) {
                boolean insertInventorySuccess = playerEntity.getInventory()
                                                             .add(itemStack);
                ...
                if (insertInventorySuccess && itemStack.isEmpty()) {
                    ...
                    playerEntity.level()
                                .playSound(
                                    ...
                                    SoundEvents.ITEM_PICKUP, SoundSource.PLAYERS, 0.2F, ((playerEntity.getRandom()
                                                                                                      .nextFloat() -
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot.hasItem()) {
            ...
            if (index == 14) {
                ...
                this.context.execute((world, pos) -> clickedSlotStack.getItem()
                                                                     .onCraftedBy(clickedSlotStack, world, player));
