...
public interface InkPowered {
    ...
    default void addInkPoweredTooltip(List<Component> tooltip) {
        if (canUseClient()) {
            if (getUsedColors().size() > 1) {
                tooltip.add(Component.translatable("pastel.tooltip.ink_powered.prefix")
                ...
                tooltip.add(Component.translatable(
                                         "pastel.tooltip.ink_powered.consume", getUsedColors().get(0)
                                                                                              .getColoredInkName()
                                     )
                                     .withStyle(ChatFormatting.GRAY));
            }
        ...
        if (stack.getItem() instanceof InkStorageItem<?> inkStorageItem) {
            ...
            if (drained > 0) {
                if (player instanceof ServerPlayer serverPlayerEntity) {
                    PastelAdvancementCriteria.INK_CONTAINER_INTERACTION.trigger(
                        serverPlayerEntity, stack, inkStorage, color, -amount);
                }
    ...
    static boolean tryDrainEnergy(@NotNull Player player, @NotNull InkCost inkCost, float costModifier) {
        return tryDrainEnergy(
            player, inkCost.color(), Support.chanceRound(inkCost.cost() * costModifier, player.getRandom()));
    }
    ...
    static long getAvailableInk(@NotNull Player player, InkColor color) {
        ...
        // trinket slots
        available += getCurioInkSlots(player).mapToLong(stack -> tryGetEnergy(stack, color))
                                             .sum();
