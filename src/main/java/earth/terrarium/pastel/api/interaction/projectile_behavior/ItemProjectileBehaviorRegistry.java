...
public class ItemProjectileBehaviorRegistry {
    public static final Codec<ProjectileBehaviorType<?>> TYPE_CODEC = ResourceLocation.CODEC
        .comapFlatMap(
            ItemProjectileBehaviorRegistry::decode,
            ProjectileBehaviorType::id
        );

    public static final Codec<ItemProjectileBehavior> CODEC = TYPE_CODEC
        .dispatch(
            ItemProjectileBehavior::type,
    ...
    public static void register(ProjectileBehaviorType<?> type) {
        if (BEHAVIOR_TYPES.containsKey(type.id())) {
            throw new IllegalArgumentException(
                "Projectile behavior type with id " + type.id() + " is already registered."
            );
        }
    ...
    private static DataResult<? extends ProjectileBehaviorType<?>> decode(ResourceLocation id) {
        return Optional
            .ofNullable(BEHAVIOR_TYPES.get(id))
