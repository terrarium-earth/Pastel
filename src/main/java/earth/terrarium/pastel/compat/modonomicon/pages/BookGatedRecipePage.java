...
public class BookGatedRecipePage<T extends GatedRecipe<?>> extends BookRecipePage<T> implements GatedGuidebookPage {
    ...
    public BookGatedRecipePage(
        RecipeType<T> recipeType, ResourceLocation pageType, BookTextHolder title1, ResourceLocation recipeId1,
        BookTextHolder title2, ResourceLocation recipeId2, BookTextHolder text, String anchor, BookCondition condition
    ) {
        super(recipeType, title1, recipeId1, title2, recipeId2, text, anchor, condition);
        this.pageType = pageType;
    }

    public static BookCondition getConditionWithRecipes(
        BookCondition condition, ResourceLocation recipeId1, ResourceLocation recipeId2) {
    ...
    public static <T extends GatedRecipe<?>> BookGatedRecipePage<T> fromJson(
        ResourceLocation entryId, ResourceLocation pageType, RecipeType<T> recipeType, JsonObject json,
        boolean supportsTwoRecipesOnOnePage, HolderLookup.Provider provider
    ) {
        ...
        if (supportsTwoRecipesOnOnePage) {
            ...
            ResourceLocation recipeId1 = json.has("recipe_id") ? ResourceLocation.tryParse(
                GsonHelper.getAsString(json, "recipe_id")) : null;
            ResourceLocation recipeId2 = json.has("recipe_id2") ? ResourceLocation.tryParse(
                GsonHelper.getAsString(json, "recipe_id2")) : null;
            condition = skipRecipeUnlockCheck ? condition : getConditionWithRecipes(condition, recipeId1, recipeId2);
            return new BookGatedRecipePage<>(
                recipeType, pageType, title1, recipeId1, title2, recipeId2, text, anchor, condition);
        } else {
            var title = BookGsonHelper.getAsBookTextHolder(json, "title", BookTextHolder.EMPTY, provider);
            ResourceLocation recipeId = json.has("recipe_id") ? ResourceLocation.tryParse(
                GsonHelper.getAsString(json, "recipe_id")) : null;
            condition = skipRecipeUnlockCheck ? condition : getConditionWithRecipes(condition, recipeId, null);
            return new BookGatedRecipePage<>(
                recipeType, pageType, title, recipeId, BookTextHolder.EMPTY, null, text, anchor, condition);
        }
    }

    public static <T extends GatedRecipe<?>> BookGatedRecipePage<T> fromNetwork(
        ResourceLocation pageType, RecipeType<T> recipeType, RegistryFriendlyByteBuf buffer) {
        var common = BookRecipePage.commonFromNetwork(buffer);
        var anchor = buffer.readUtf();
        var condition = BookCondition.fromNetwork(buffer);
        return new BookGatedRecipePage<>(
            recipeType, pageType, common.title1(), common.recipeId1(), common.title2(), common.recipeId2(),
            common.text(), anchor, condition
        );
    }
