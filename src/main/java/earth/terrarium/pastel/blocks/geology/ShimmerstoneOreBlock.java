...
public class ShimmerstoneOreBlock extends CloakedOreBlock {
    ...
    public static final MapCodec<ShimmerstoneOreBlock> CODEC = RecordCodecBuilder
        .mapCodec(
            instance -> instance
                .group(
                    IntProvider
        ...
        if (!world.isClientSide() && !entity.isSteppingCarefully() && random.nextInt(3) == 0) {
            PlayParticleAroundBlockSidesPayload
                .playParticleAroundBlockSides(
                    ...
                    (player) -> BlockHiddenType.isVisible(state, player),
                    Direction.UP
                );
        ...
        if (!world.isClientSide()) {
            PlayParticleAroundBlockSidesPayload
                .playParticleAroundBlockSides(
                    ...
                    new Vec3(0, 0.05, 0),
                    PastelParticleTypes.SHIMMERSTONE_SPARKLE,
                    serverPlayer -> BlockHiddenType.isVisible(state, serverPlayer),
                    Direction.values()
                );
        }
