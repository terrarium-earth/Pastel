...
public class EnchanterCraftingRecipe extends EnchanterRecipe {
	...
	public void consumeIngredients(EnchanterBlockEntity enchanter, HolderLookup.Provider lookup, double scaling) {
		...
		if (!noDiscounts) {
			output = 1 / enchanter.getUpgrades().getEffectiveValue(Upgradeable.UpgradeType.EFFICIENCY);
		}
	...
	public static class Serializer implements RecipeSerializer<EnchanterCraftingRecipe> {
		...
		public static final MapCodec<EnchanterCraftingRecipe> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
				...
				Codec.BOOL.optionalFieldOf("disable_yield_and_efficiency_upgrades", false).forGetter(recipe -> recipe.noDiscounts),
				Codec.BOOL.optionalFieldOf("copy_components", false).forGetter(recipe -> recipe.copyComponents)
		).apply(i, EnchanterCraftingRecipe::new));
		
		public static final StreamCodec<RegistryFriendlyByteBuf, EnchanterCraftingRecipe> STREAM_CODEC = PacketCodecHelper.tuple(
