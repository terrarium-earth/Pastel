...
public class PastelPresentUnpackBehaviors {
    ...
    public static void register() {
        ...
        PresentBlock.registerBehavior(
            PastelItems.STORM_STONE.getId(), (stack, presentBlockEntity, world, pos, random) -> {
        ...
        PresentBlock.registerBehavior(
            PastelBlocks.INCANDESCENT_AMALGAM.getId(), (stack, presentBlockEntity, world, pos, random) -> {
        ...
        PresentBlock.registerBehavior(
            BuiltInRegistries.ITEM.getKey(Items.FIREWORK_ROCKET), (stack, presentBlockEntity, world, pos, random) -> {
                Vec3 centerPos = Vec3.atLowerCornerOf(pos);
                for (int i = 0; i < stack.getCount(); i++) {
                    FireworkRocketEntity fireworkRocketEntity = new FireworkRocketEntity(
                        world, presentBlockEntity.getOwnerIfOnline(), centerPos.x + 0.35 + random.nextFloat() * 0.3,
                        centerPos.y + 0.35 + random.nextFloat() * 0.3, centerPos.z + 0.35 + random.nextFloat() * 0.3,
                        stack
                    );
        ...
        PresentBlock.registerBehavior(
            BuiltInRegistries.ITEM.getKey(Items.GOAT_HORN), (stack, presentBlockEntity, world, pos, random) -> {
                Optional<Holder<Instrument>> optional = ((GoatHornItemAccessor) stack.getItem()).invokeGetInstrument(
                    stack);
        ...
        PresentBlock.registerBehavior(
            BuiltInRegistries.ITEM.getKey(Items.BELL), (stack, presentBlockEntity, world, pos, random) -> {
        ...
        PresentBlock.registerBehavior(
            BuiltInRegistries.ITEM.getKey(Items.TNT), (stack, presentBlockEntity, world, pos, random) -> {
                if (stack.getCount() > 0) {
                    PrimedTnt tntEntity = null;
                    for (int i = 0; i < stack.getCount(); i++) {
                        tntEntity = new PrimedTnt(
                            world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5,
                            presentBlockEntity.getOwnerIfOnline()
                        );
                        world.addFreshEntity(tntEntity);
                    }
                    world.playSound(
                        null, tntEntity.getX(), tntEntity.getY(), tntEntity.getZ(), SoundEvents.TNT_PRIMED,
        ...
        PresentBlock.registerBehavior(
            BuiltInRegistries.ITEM.getKey(Items.EXPERIENCE_BOTTLE), (stack, presentBlockEntity, world, pos, random) -> {
                int totalXP = 0;
                for (int i = 0; i < stack.getCount(); i++) {
                ...
                world.levelEvent(
                    LevelEvent.PARTICLES_SPELL_POTION_SPLASH, pos, new PotionContents(Potions.WATER).getColor());
        ...
        PresentBlock.registerBehavior(
            BuiltInRegistries.ITEM.getKey(Items.EGG), (stack, presentBlockEntity, world, pos, random) -> {
                int chickenCount
                    = stack.getCount(); // every egg hatches, unlike via EggEntity. New chicken farm just dropped?
        ...
        PresentBlock.registerBehavior(
            PastelBlocks.MEMORY.getId(), (stack, presentBlockEntity, world, pos, random) -> {
