...
public class PastelRenderHelper {
    ...
    public static void renderLineTo(
        final PoseStack matrices, final MultiBufferSource vertexConsumers, final float[] color, final BlockPos thisPos,
        final BlockPos pos
    ) {
        ...
        final double[] billBoard = billboard(
            (vec.x) * 1, (vec.y) * 1, (vec.z) * 1, camera.getPosition().x, camera.getPosition().y,
            camera.getPosition().z, axis.x, axis.y, axis.z
        );
        ...
        renderBeamFace(
            billBoard, model, entry, buffer, color[1], color[2], color[3], color[0], dist, 0, 0, BEAM_WIDTH,
            -BEAM_WIDTH, 0, 0.5F, 0, 1
        );
        renderBeamFace(
            billBoard, model, entry, buffer, color[1], color[2], color[3], color[0], dist, -BEAM_WIDTH, -BEAM_WIDTH, 0,
            0, 0.5F, 1, 0, 1
        );
    ...
    private static void renderBeamFace(
        final double[] mat, final Matrix4f positionMatrix, final PoseStack.Pose entry, final VertexConsumer vertices,
        final float red, final float green, final float blue, final float alpha, final float length, final float x1,
        final float z1, final float x2, final float z2, final float u1, final float u2, final float v1, final float v2
    ) {
    ...
    private static void renderBeamVertex(
        final double[] mat, final Matrix4f positionMatrix, final PoseStack.Pose entry, final VertexConsumer vertices,
        final float red, final float green, final float blue, final float alpha, final float x, final float y,
        final float z, final float u, final float v
    ) {
        final Vec3 transform = transform(new Vec3(x, y, z), mat);
        vertices.addVertex(positionMatrix, (float) transform.x, (float) transform.y, (float) transform.z)
    ...
    private static Vec3 transform(final Vec3 vec, final double[] mat) {
        final double x = mat[matIndex(0, 0)] * vec.x + mat[matIndex(0, 1)] * vec.y + mat[matIndex(0, 2)] * vec.z +
                         mat[matIndex(0, 3)];
        final double y = mat[matIndex(1, 0)] * vec.x + mat[matIndex(1, 1)] * vec.y + mat[matIndex(1, 2)] * vec.z +
                         mat[matIndex(1, 3)];
        final double z = mat[matIndex(2, 0)] * vec.x + mat[matIndex(2, 1)] * vec.y + mat[matIndex(2, 2)] * vec.z +
                         mat[matIndex(2, 3)];
    ...
    private static double[] billboard(
        final double objX, final double objY, final double objZ, final double targetX, final double targetY,
        final double targetZ, final double upX, final double upY, final double upZ
    ) {
