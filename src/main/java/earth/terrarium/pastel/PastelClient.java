...
public class PastelClient implements RevealingCallback, ClientAdvancementPacketCallback {
    ...
    public PastelClient(IEventBus pastelBus, ModContainer container) {
        ...
        container.registerExtensionPoint(
            IConfigScreenFactory.class, (v, parent) -> AutoConfig.getConfigScreen(PastelConfig.class, parent)
                                                                 .get()
        );
        ...
        if (!isJoinPacket) {
            for (Block block : blocks) {
                if (BuiltInRegistries.BLOCK.getKey(block)
                    ...
                    RevelationToast.showRevelationToast(
                        Minecraft.getInstance(), new ItemStack(PastelBlocks.PEDESTAL_BASIC_AMETHYST.get()
                                                                                                   .asItem()),
                        PastelSoundEvents.NEW_REVELATION
                    );
                    break;
                }
            }
    ...
    @Override
    public void onClientAdvancementPacket(
        Set<ResourceLocation> gottenAdvancements, Set<ResourceLocation> removedAdvancements, boolean isJoinPacket) {
        if (!isJoinPacket) {
