...
public interface OmniAcceleratorProjectile {
    ...
    static OmniAcceleratorProjectile get(ItemStack stack) {
        for (
            Tuple<ItemPredicate, OmniAcceleratorProjectile> entry : PROJECTILES
        ) {
            if (entry
                .getA()
                .test(stack)) {
     ...
     * @return The created projectile. If not null, the fired stack will be decremented and the getSoundEffect()
     *         sound will play
     */
    @Nullable Entity createProjectile(ItemStack stack, LivingEntity shooter, Level world, ItemStack shotFrom);
        ...
        Vec3 vec3d = (new Vec3(x, y, z))
            ...
            .add(
                projectile
                    .level()
                    .getRandom()
                    .triangle(0.0, 0.0172275 * (double) divergence),
                projectile
                    .level()
                    .getRandom()
                    .triangle(0.0, 0.0172275 * (double) divergence),
    ...
    static void setVelocity(
        ...
        projectile
            .setDeltaMovement(
                projectile
                    .getDeltaMovement()
                    .add(vec3d.x, shooter.onGround() ? 0.0 : vec3d.y, vec3d.z)
            );
    }
