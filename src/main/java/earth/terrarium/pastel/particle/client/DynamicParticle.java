...
public class DynamicParticle extends TextureSheetParticle {
    ...
    public static class Factory<P extends DynamicParticleEffect> implements ParticleProvider<P> {
        ...
        public Particle createParticle(
            P parameters, ClientLevel clientWorld, double x, double y, double z, double velocityX, double velocityY,
            double velocityZ
        ) {
            ...
            SpriteSet dynamicProvider = ((ParticleManagerAccessor) client.particleEngine).getSpriteSets()
                                                                                         .get(particleTypeIdentifier);
            if (dynamicProvider == null) {
                PastelCommon.logError("Trying to use a non-existent sprite provider for particle spawner particle: " +
