...
public class PastelEffectLang {
    public static void addTranslations(PastelLanguageProvider provider) {
        for (
            var effect : PastelMobEffects.REGISTER.getEntries()
        ) {
        ...
        provider
            .add(
                "effect.pastel.effect_prolonging.desc",
                "Increases the duration of other status effects applied to you."
            );
        provider
            .add(
        ...
        provider
            .add(
                ...
                "Buffs damage, attack speed, movement speed, and knockback resistance with each kill. Not scoring a kill " + "in 10 seconds will debuff you instead."
            );
        provider
            .add(
        ...
        provider
            .add(
                ...
                "Makes you lighter, increasing jump height, slowing falls, and at high levels granting immunity to fall " + "damage."
            );
        provider.add("effect.pastel.magic_annulation.desc", "Reduces incoming magic damage by 1 point per level.");
        provider
            .add(
                "effect.pastel.projectile_rebound.desc",
                "Grants a small chance to defend against incoming projectiles."
            );
        provider
            .add(
                "effect.pastel.scarred.desc",
                "Stops natural health regeneration and prevents you from sprinting."
            );
