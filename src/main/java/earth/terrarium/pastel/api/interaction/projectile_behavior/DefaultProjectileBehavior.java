...
public record DefaultProjectileBehavior() implements ItemProjectileBehavior {
    ...
    public ItemStack onEntityHit(
        ...
        if (stack.is(ItemTags.CREEPER_IGNITERS) && target instanceof Creeper creeperEntity) {
            ...
            SoundEvent soundEvent = stack.is(Items.FIRE_CHARGE) ? SoundEvents.FIRECHARGE_USE
                                                                : SoundEvents.FLINTANDSTEEL_USE;
            world.playSound(
                null, projectile.getX(), projectile.getY(), projectile.getZ(), soundEvent, projectile.getSoundSource(),
                1.0F, world.getRandom()
                           .nextFloat() * 0.4F + 0.8F
            );
            ...
            if (stack.isDamageableItem() && world instanceof ServerLevel serverWorld) {
                stack.hurtAndBreak(
                    1, serverWorld, null, item -> {
                    }
                );
            } else if (!stack.has(DataComponents.UNBREAKABLE)) {
                stack.shrink(
                    1); // In Vanilla unbreakable Flint & Steel is not handled correctly and therefore consumed. This
        ...
        if (target instanceof LivingEntity livingTarget) {
            ...
            if (owner instanceof Player playerOwner && stack.interactLivingEntity(
                                                                playerOwner, livingTarget, InteractionHand.MAIN_HAND)
                                                            .consumesAction()) {
                return stack;
            }
    ...
    public ItemStack onBlockHit(
        ...
        try {
            ...
            PastelCommon.logError("Item Projectile failed to use item " + stack.getItem()
                                                                               .getDescription());
            e.printStackTrace();
        }
