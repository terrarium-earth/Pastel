...
public class CrystallarieumDisplay extends PastelDisplay {
    ...
    public CrystallarieumDisplay(@NotNull RecipeHolder<CrystallarieumRecipe> recipe) {
        ...
        for (
            BlockState state : recipe.value().getGrowthStages()
        ) {
    ...
    public static List<EntryIngredient> outputs(CrystallarieumRecipe recipe) {
        ...
        for (
            ItemStack additionalOutput : recipe.getAdditionalResults()
        ) {
            outputs.add(EntryIngredients.of(additionalOutput));
        }

        for (
            BlockState growthStageState : recipe.getGrowthStages()
        ) {
    ...
    public boolean isUnlocked() {
        ...
        return DatabankUtils
            .hasAdvancement(client.player, CrystallarieumRecipe.UNLOCK_IDENTIFIER) && super.isUnlocked();
    }
