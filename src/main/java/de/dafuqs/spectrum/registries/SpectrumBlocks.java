...
public class SpectrumBlocks {
	...
	public static final Block FUSION_SHRINE_BASALT = register(cutout(singleton(blockWithItem("fusion_shrine_basalt", new FusionShrineBlock(craftingBlock(MapColor.BLACK, BlockSoundGroup.BASALT).luminance(value -> value.get(FusionShrineBlock.LIGHT_LEVEL))), IS.of(1), DyeColor.GRAY), SpectrumTexturedModels.FUSION_SHRINE)));
	public static final Block FUSION_SHRINE_CALCITE = register(cutout(singleton(blockWithItem("fusion_shrine_calcite", new FusionShrineBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE).luminance(value -> value.get(FusionShrineBlock.LIGHT_LEVEL))), IS.of(1), DyeColor.GRAY), SpectrumTexturedModels.FUSION_SHRINE)));
	
	public static final Block ENCHANTER = register(cutout(singleton(blockWithItem("enchanter", new EnchanterBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE)), IS.of(1), DyeColor.PURPLE), ModelIds::getBlockModelId)));
	...
	public static final Block POTION_WORKSHOP = register(translucent(defaultNorthHorizontalFacing(blockWithItem("potion_workshop", new PotionWorkshopBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE)), IS.of(1), DyeColor.PURPLE), ModelIds::getBlockModelId)));
	public static final Block SPIRIT_INSTILLER = register(singleton(blockWithItem("spirit_instiller", new SpiritInstillerBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE)), IS.of(1), DyeColor.WHITE), ModelIds::getBlockModelId));
	public static final CrystallarieumBlock CRYSTALLARIEUM = register(translucent(blockWithItem("crystallarieum", new CrystallarieumBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE)), IS.of(1), DyeColor.BROWN)).withPredefinedItemModel());
	public static final Block CINDERHEARTH = register(defaultNorthHorizontalFacing(blockWithItem("cinderhearth", new CinderhearthBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE)), IS.of(1).fireproof(), DyeColor.ORANGE), ModelIds::getBlockModelId));
	
	public static final Block COLOR_PICKER = register(cutout(defaultWestHorizontalFacing(blockWithItem("color_picker", new ColorPickerBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE)), IS.of(8), DyeColor.GREEN), ModelIds::getBlockModelId)));
	public static final Block CRYSTAL_APOTHECARY = register(singleton(blockWithItem("crystal_apothecary", new CrystalApothecaryBlock(craftingBlock(MapColor.TERRACOTTA_WHITE, BlockSoundGroup.CALCITE)), IS.of(8), DyeColor.GREEN), ModelIds::getBlockModelId));
	...
	public static <T extends SpectrumClusterBlock> BlockRegistrar<T> cluster(BlockRegistrar<T> registrar, Model model) {
		return cutout(registrar).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(TexturedModel.makeFactory(TextureMap::cross, model).upload(block, ctx.modelCollector))).coordinate(createUpDefaultFacingVariantMap())).withBlockItemModel((ctx, block) -> {
	...
	public static final SpectrumGemstoneBlock MOONSTONE_BLOCK = register(blockWithItem("moonstone_block", new SpectrumGemstoneBlock(gemstoneBlock(MapColor.WHITE, SpectrumBlockSoundGroups.MOONSTONE_BLOCK), SpectrumSoundEvents.BLOCK_MOONSTONE_BLOCK_HIT, SpectrumSoundEvents.BLOCK_MOONSTONE_BLOCK_CHIME), DyeColor.WHITE).withBlockModel(SpectrumModelProvider::simpleMirroredBlockModel));
	
	public static final ColoredFallingBlock TOPAZ_POWDER_BLOCK = register(simple(blockWithItem("topaz_powder_block", new ColoredFallingBlock(new ColorCode(DyeColor.CYAN.getFireworkColor()), AbstractBlock.Settings.copy(SAND).mapColor(MapColor.CYAN)), DyeColor.CYAN)));
	public static final ColoredFallingBlock AMETHYST_POWDER_BLOCK = register(simple(blockWithItem("amethyst_powder_block", new ColoredFallingBlock(new ColorCode(DyeColor.MAGENTA.getFireworkColor()), AbstractBlock.Settings.copy(SAND).mapColor(MapColor.MAGENTA)), DyeColor.MAGENTA)));
	public static final ColoredFallingBlock CITRINE_POWDER_BLOCK = register(simple(blockWithItem("citrine_powder_block", new ColoredFallingBlock(new ColorCode(DyeColor.YELLOW.getFireworkColor()), AbstractBlock.Settings.copy(SAND).mapColor(MapColor.YELLOW)), DyeColor.YELLOW)));
	public static final ColoredFallingBlock ONYX_POWDER_BLOCK = register(simple(blockWithItem("onyx_powder_block", new ColoredFallingBlock(new ColorCode(DyeColor.BLACK.getFireworkColor()), AbstractBlock.Settings.copy(SAND).mapColor(MapColor.BLACK)), DyeColor.BLACK)));
	public static final ColoredFallingBlock MOONSTONE_POWDER_BLOCK = register(simple(blockWithItem("moonstone_powder_block", new ColoredFallingBlock(new ColorCode(DyeColor.WHITE.getFireworkColor()), AbstractBlock.Settings.copy(SAND).mapColor(MapColor.WHITE)), DyeColor.WHITE)));
	
	public static final Block VEGETAL_BLOCK = register(translucent(singleton(blockWithItem("vegetal_block", new Block(settings(MapColor.PALE_GREEN, BlockSoundGroup.FUNGUS, 2.0F).nonOpaque()), DyeColor.GREEN), TexturedModel.makeFactory(TextureMap::texture, SpectrumModels.TRANSLUCENT_OUTER1))));
	public static final Block NEOLITH_BLOCK = register(blockWithItem("neolith_block", new SpectrumFacingBlock(settings(MapColor.PURPLE, BlockSoundGroup.COPPER, 6.0F).requiresTool().instrument(NoteBlockInstrument.BASEDRUM).luminance(state -> 13).postProcess(SpectrumBlocks::always).emissiveLighting(SpectrumBlocks::always)), DyeColor.PINK).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(TexturedModel.CUBE_BOTTOM_TOP.upload(block, ctx.modelCollector))).coordinate(createUpDefaultFacingVariantMap())));
	...
	public static final Block BLACKSLAG = register(blockWithItem("blackslag", new BlackslagBlock(blackslag(BlockSoundGroup.DEEPSLATE)), DyeColor.BLACK).withBlockModel((ctx, block) -> createMirroredVariantsSupplier(block, TexturedModel.END_FOR_TOP_CUBE_COLUMN, SpectrumTexturedModels.CUBE_COLUMN_MIRRORED, ctx.modelCollector).coordinate(createAxisRotatedVariantMap())));
	...
	public static final Block INFESTED_BLACKSLAG = register(parented(blockWithItem("infested_blackslag", new InfestedBlock(BLACKSLAG, blackslag(BlockSoundGroup.DEEPSLATE)), DyeColor.BLACK), b -> BLACKSLAG));
	...
	public static final Block PYRITE_PILE = register(axisRotated(blockWithItem("pyrite_pile", new PillarBlock(Settings.copy(PYRITE)), DyeColor.BROWN), TexturedModel.CUBE_COLUMN));
	public static final Block PYRITE_PLATING = register(simple(blockWithItem("pyrite_plating", new Block(Settings.copy(PYRITE)), DyeColor.BROWN)));
	public static final Block PYRITE_TUBING = register(axisRotated(blockWithItem("pyrite_tubing", new PillarBlock(Settings.copy(PYRITE)), DyeColor.BROWN), TexturedModel.CUBE_COLUMN));
	public static final Block PYRITE_RELIEF = register(axisRotated(blockWithItem("pyrite_relief", new PillarBlock(Settings.copy(PYRITE)), DyeColor.BROWN), SpectrumTexturedModels.cubeColumn(b -> b, "_side", b -> PYRITE_TUBING, "_top")));
	public static final Block PYRITE_STACK = register(simple(blockWithItem("pyrite_stack", new Block(Settings.copy(PYRITE)), DyeColor.BROWN)));
	public static final Block PYRITE_PANELING = register(singleton(blockWithItem("pyrite_paneling", new Block(Settings.copy(PYRITE)), DyeColor.BROWN), SpectrumTexturedModels.cubeColumn(b -> b, "", b -> PYRITE_PLATING, "")));
	public static final Block PYRITE_VENT = register(singleton(blockWithItem("pyrite_vent", new Block(Settings.copy(PYRITE)), DyeColor.BROWN), SpectrumTexturedModels.cubeColumn(b -> b, "", b -> PYRITE_PLATING, "")));
	...
	public static final Block DRAGONBONE = register(axisRotated(blockWithItem("dragonbone", new DragonboneBlock(Settings.copy(BONE_BLOCK).strength(-1.0F, 22.0F).pistonBehavior(PistonBehavior.BLOCK)), DyeColor.GREEN), TexturedModel.CUBE_COLUMN));
	public static final Block CRACKED_DRAGONBONE = register(axisRotated(blockWithItem("cracked_dragonbone", new PillarBlock(Settings.copy(BONE_BLOCK).strength(100.0F, 1200.0F).pistonBehavior(PistonBehavior.BLOCK)), DyeColor.GREEN), TexturedModel.CUBE_COLUMN));
	...
	public static final Block BONE_ASH_SHINGLES = register(blockWithItem("bone_ash_shingles", new ShinglesBlock(AbstractBlock.Settings.copy(POLISHED_BONE_ASH).nonOpaque()), DyeColor.CYAN).withBlockModel((ctx, block) -> createVariantsSupplier(block, ModelIds.getBlockModelId(block)).coordinate(createEastDefaultHorizontalFacingVariantMap())));
	...
	public static final Block BLACK_MATERIA = register(simple(blockWithItem("black_materia", new BlackMateriaBlock(settings(MapColor.TERRACOTTA_BLACK, BlockSoundGroup.SAND, 0.0F).instrument(NoteBlockInstrument.SNARE).ticksRandomly()), DyeColor.GRAY)));
	...
	public static final PrimordialFireBlock PRIMORDIAL_FIRE = register(cutout(block("primordial_fire", new PrimordialFireBlock(Settings.copy(FIRE).mapColor(MapColor.PURPLE).luminance((state) -> 10)))).withBlockModel((ctx, block) -> {
		...
		return MultipartBlockStateSupplier.create(block)
				.with(noSides, createModelVariant(SpectrumModels.FIRE_FLOOR.upload(block, "_floor0", fire0, ctx.modelCollector)), createModelVariant(SpectrumModels.FIRE_FLOOR.upload(block, "_floor1", fire1, ctx.modelCollector)))
				.with(When.create().set(PrimordialFireBlock.UP, true), createModelVariant(SpectrumModels.FIRE_UP.upload(block, "_up0", fire0, ctx.modelCollector)), createModelVariant(SpectrumModels.FIRE_UP.upload(block, "_up1", fire1, ctx.modelCollector)), createModelVariant(SpectrumModels.FIRE_UP_ALT.upload(block, "_up_alt0", fire0, ctx.modelCollector)), createModelVariant(SpectrumModels.FIRE_UP_ALT.upload(block, "_up_alt1", fire1, ctx.modelCollector)))
				.with(When.anyOf(noSides, When.create().set(PrimordialFireBlock.NORTH, true)), createModelVariant(side0), createModelVariant(side1), createModelVariant(sideAlt0), createModelVariant(sideAlt1))
				.with(When.anyOf(noSides, When.create().set(PrimordialFireBlock.SOUTH, true)), createModelVariant(side0).put(VariantSettings.Y, VariantSettings.Rotation.R180), createModelVariant(side1).put(VariantSettings.Y, VariantSettings.Rotation.R180), createModelVariant(sideAlt0).put(VariantSettings.Y, VariantSettings.Rotation.R180), createModelVariant(sideAlt1).put(VariantSettings.Y, VariantSettings.Rotation.R180))
				.with(When.anyOf(noSides, When.create().set(PrimordialFireBlock.WEST, true)), createModelVariant(side0).put(VariantSettings.Y, VariantSettings.Rotation.R270), createModelVariant(side1).put(VariantSettings.Y, VariantSettings.Rotation.R270), createModelVariant(sideAlt0).put(VariantSettings.Y, VariantSettings.Rotation.R270), createModelVariant(sideAlt1).put(VariantSettings.Y, VariantSettings.Rotation.R270))
				.with(When.anyOf(noSides, When.create().set(PrimordialFireBlock.EAST, true)), createModelVariant(side0).put(VariantSettings.Y, VariantSettings.Rotation.R90), createModelVariant(side1).put(VariantSettings.Y, VariantSettings.Rotation.R90), createModelVariant(sideAlt0).put(VariantSettings.Y, VariantSettings.Rotation.R90), createModelVariant(sideAlt1).put(VariantSettings.Y, VariantSettings.Rotation.R90));
	}));
	...
	public static final Block PRIMORDIAL_TORCH = register(cutout(singleton(blockWithItem("primordial_torch", new TorchBlock(SpectrumParticleTypes.PRIMORDIAL_FLAME, Settings.copy(SOUL_TORCH).luminance(s -> 13)), block -> new VerticallyAttachableBlockItem(block, PRIMORDIAL_WALL_TORCH, IS.of(), Direction.DOWN), DyeColor.ORANGE), ModelIds::getBlockModelId).withItemModel(SpectrumModelProvider::registerItemModel)));
	
	public static <T extends Block> BlockRegistrar<T> moonstoneChiseled(BlockRegistrar<T> registrar, Identifier capTexture) {
		return registrar.withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, block, "_down")).withBlockModel((ctx, block) -> {
	...
	public static final Block SMOOTH_BASALT_SLAB = register(blockWithItem("smooth_basalt_slab", new SlabBlock(AbstractBlock.Settings.copy(BASALT)), DyeColor.BROWN));
	public static final Block SMOOTH_BASALT_WALL = register(blockWithItem("smooth_basalt_wall", new WallBlock(AbstractBlock.Settings.copy(BASALT)), DyeColor.BROWN));
	...
	public static final Block CALCITE_STAIRS = register(blockWithItem("calcite_stairs", new StairsBlock(CALCITE.getDefaultState(), AbstractBlock.Settings.copy(CALCITE)), DyeColor.BROWN));
	public static final Block CALCITE_SLAB = register(blockWithItem("calcite_slab", new SlabBlock(AbstractBlock.Settings.copy(CALCITE)), DyeColor.BROWN));
	public static final Block CALCITE_WALL = register(blockWithItem("calcite_wall", new WallBlock(AbstractBlock.Settings.copy(CALCITE)), DyeColor.BROWN));
	public static final BlockFamily CALCITE_FAMILY = registerBlockFamilyExceptBase(new BlockFamily.Builder(CALCITE).stairs(CALCITE_STAIRS).slab(CALCITE_SLAB).wall(CALCITE_WALL).build(), TexturedModel.CUBE_ALL);
	...
	public static final Block ETHEREAL_PLATFORM = register(translucent(simple(blockWithItem("ethereal_platform", new EtherealPlatformBlock(gemstoneGlass(BlockSoundGroup.AMETHYST_BLOCK, MapColor.CLEAR).pistonBehavior(PistonBehavior.NORMAL)), DyeColor.LIGHT_GRAY))));
	public static final Block UNIVERSE_SPYHOLE = register(translucent(simple(blockWithItem("universe_spyhole", new TransparentBlock(settings(MapColor.CLEAR, SpectrumBlockSoundGroups.CITRINE_BLOCK, 1.5F).requiresTool().blockVision(SpectrumBlocks::never)), DyeColor.LIGHT_GRAY))));
	...
	public static final Block TOPAZ_CHIME = register(translucent(singleton(blockWithItem("topaz_chime", new GemstoneChimeBlock(chime(TOPAZ_CLUSTER), SpectrumSoundEvents.BLOCK_TOPAZ_BLOCK_CHIME, SpectrumParticleTypes.CYAN_SPARKLE_RISING), DyeColor.CYAN), SpectrumTexturedModels.CHIME)));
	public static final Block AMETHYST_CHIME = register(translucent(singleton(blockWithItem("amethyst_chime", new GemstoneChimeBlock(chime(AMETHYST_CLUSTER), SoundEvents.BLOCK_AMETHYST_BLOCK_CHIME, SpectrumParticleTypes.MAGENTA_SPARKLE_RISING), DyeColor.MAGENTA), SpectrumTexturedModels.CHIME)));
	public static final Block CITRINE_CHIME = register(translucent(singleton(blockWithItem("citrine_chime", new GemstoneChimeBlock(chime(CITRINE_CLUSTER), SpectrumSoundEvents.BLOCK_CITRINE_BLOCK_CHIME, SpectrumParticleTypes.YELLOW_SPARKLE_RISING), DyeColor.YELLOW), SpectrumTexturedModels.CHIME)));
	public static final Block ONYX_CHIME = register(translucent(singleton(blockWithItem("onyx_chime", new GemstoneChimeBlock(chime(ONYX_CLUSTER), SpectrumSoundEvents.BLOCK_ONYX_BLOCK_CHIME, SpectrumParticleTypes.BLACK_SPARKLE_RISING), DyeColor.BLACK), SpectrumTexturedModels.CHIME)));
	public static final Block MOONSTONE_CHIME = register(translucent(singleton(blockWithItem("moonstone_chime", new GemstoneChimeBlock(chime(MOONSTONE_CLUSTER), SpectrumSoundEvents.BLOCK_MOONSTONE_BLOCK_CHIME, SpectrumParticleTypes.WHITE_SPARKLE_RISING), DyeColor.WHITE), SpectrumTexturedModels.CHIME)));
	...
	public static final Block RADIANT_SEMI_PERMEABLE_GLASS = register(translucent(parented(blockWithItem("radiant_semi_permeable_glass", new AlternatePlayerOnlyGlassBlock(AbstractBlock.Settings.copy(RADIANT_GLASS), RADIANT_GLASS, false), DyeColor.YELLOW), b -> RADIANT_GLASS)));
	public static final Block TOPAZ_SEMI_PERMEABLE_GLASS = register(translucent(parented(blockWithItem("topaz_semi_permeable_glass", new GemstonePlayerOnlyGlassBlock(AbstractBlock.Settings.copy(TOPAZ_GLASS), BuiltinGemstoneColor.CYAN), DyeColor.CYAN), b -> TOPAZ_GLASS)));
	public static final Block AMETHYST_SEMI_PERMEABLE_GLASS = register(translucent(parented(blockWithItem("amethyst_semi_permeable_glass", new GemstonePlayerOnlyGlassBlock(AbstractBlock.Settings.copy(AMETHYST_GLASS), BuiltinGemstoneColor.MAGENTA), DyeColor.MAGENTA), b -> AMETHYST_GLASS)));
	public static final Block CITRINE_SEMI_PERMEABLE_GLASS = register(translucent(parented(blockWithItem("citrine_semi_permeable_glass", new GemstonePlayerOnlyGlassBlock(AbstractBlock.Settings.copy(CITRINE_GLASS), BuiltinGemstoneColor.YELLOW), DyeColor.YELLOW), b -> CITRINE_GLASS)));
	...
	public static final RegistryKey<Block> GLISTERING_MELON_STEM = cutout(new BlockRegistrar<>("glistering_melon_stem").withBlock(() -> new StemBlock(GLISTERING_MELON, ATTACHED_GLISTERING_MELON_STEM, SpectrumItems.GLISTERING_MELON_SEEDS, AbstractBlock.Settings.copy(MELON_STEM))).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_7).register(age -> createModelVariant(Models.STEM_GROWTH_STAGES[age].upload(block, TextureMap.stem(block), ctx.modelCollector))))).withBlockModel((ctx, block) -> {
	...
	public static final Block PRESENT = register(cutout(blockWithItem("present", new PresentBlock(AbstractBlock.Settings.copy(WHITE_WOOL)), block -> new PresentBlockItem(block, IS.of(1)), DyeColor.LIGHT_GRAY)).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(PresentBlock.VARIANT).register(variant -> createModelVariant(SpectrumModels.PRESENT.upload(block, "_" + variant.asString(), new TextureMap().put(TextureKey.TEXTURE, TextureMap.getSubId(block, "_" + variant.asString())).put(TextureKey.PARTICLE, TextureMap.getId(variant.woolBase)), ctx.modelCollector))))).withPredefinedItemModel());
	public static final Block TITRATION_BARREL = register(blockWithItem("titration_barrel", new TitrationBarrelBlock(AbstractBlock.Settings.copy(OAK_PLANKS).mapColor(MapColor.RED)), DyeColor.MAGENTA).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(createUpDefaultHorizontalFacingVariantMap()).coordinate(BlockStateVariantMap.create(TitrationBarrelBlock.BARREL_STATE).register(state -> createModelVariant(SpectrumTexturedModels.cubeBottomTop(b -> b, "_side", b -> b, "_top_" + state.asString(), b -> b, "_bottom").upload(block, state == TitrationBarrelBlock.BarrelState.EMPTY ? "" : "_" + state.asString(), ctx.modelCollector))))));
	
	public static final Block PARAMETRIC_MINING_DEVICE = register(defaultUpFacing(blockWithItem("parametric_mining_device", new ParametricMiningDeviceBlock(AbstractBlock.Settings.copy(BLACKSLAG).nonOpaque().breakInstantly()), block -> new ParametricMiningDeviceItem(block, IS.of(8)), DyeColor.RED), ModelIds::getBlockModelId).withItemModel(SpectrumModelProvider::registerItemModel));
	public static final Block THREAT_CONFLUX = register(blockWithItem("threat_conflux", new ThreatConfluxBlock(AbstractBlock.Settings.copy(BLACKSLAG).nonOpaque().breakInstantly()), block -> new ThreatConfluxItem(block, IS.of(8)), DyeColor.RED).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(ThreatConfluxBlock.ARMED).register(armed -> createModelVariant(block, armed == ThreatConfluxBlock.ArmedState.NOT_ARMED ? "" : "_armed")))));
	...
	public static final Block BOTTOMLESS_BUNDLE = register(cutout(singleton(blockWithItem("bottomless_bundle", new BottomlessBundleBlock(settings(MapColor.PALE_PURPLE, BlockSoundGroup.WOOL, 1.0F).nonOpaque().pistonBehavior(PistonBehavior.DESTROY)), block -> new BottomlessBundleItem(block, IS.of(1)), DyeColor.LIGHT_GRAY), ModelIds::getBlockModelId)).withPredefinedItemModel());
	...
	public static <T extends Block> BlockRegistrar<T> decay(BlockRegistrar<T> registrar) {
		return registrar.withBlockModel((ctx, block) -> {
			...
			return VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(DecayBlock.CONVERSION).register(DecayBlock.Conversion.NONE, createModelVariant(none)).register(DecayBlock.Conversion.DEFAULT, createModelVariant(def)).register(DecayBlock.Conversion.SPECIAL, createModelVariant(def)));
		});
	}
	...
	public static final Block TOPAZ_SUGAR_STICK = register(sugarStick(blockWithItem("topaz_sugar_stick", new SugarStickBlock(rockCandy(SMALL_TOPAZ_BUD), RockCandy.RockCandyVariant.TOPAZ), DyeColor.PINK), b -> TOPAZ_GLASS));
	public static final Block AMETHYST_SUGAR_STICK = register(sugarStick(blockWithItem("amethyst_sugar_stick", new SugarStickBlock(rockCandy(SMALL_AMETHYST_BUD), RockCandy.RockCandyVariant.AMETHYST), DyeColor.PINK), b -> AMETHYST_GLASS));
	public static final Block CITRINE_SUGAR_STICK = register(sugarStick(blockWithItem("citrine_sugar_stick", new SugarStickBlock(rockCandy(SMALL_CITRINE_BUD), RockCandy.RockCandyVariant.CITRINE), DyeColor.PINK), b -> CITRINE_GLASS));
	public static final Block ONYX_SUGAR_STICK = register(sugarStick(blockWithItem("onyx_sugar_stick", new SugarStickBlock(rockCandy(SMALL_ONYX_BUD), RockCandy.RockCandyVariant.ONYX), DyeColor.PINK), b -> ONYX_GLASS));
	public static final Block MOONSTONE_SUGAR_STICK = register(sugarStick(blockWithItem("moonstone_sugar_stick", new SugarStickBlock(rockCandy(SMALL_MOONSTONE_BUD), RockCandy.RockCandyVariant.MOONSTONE), DyeColor.PINK), b -> MOONSTONE_GLASS));
	...
	public static final Block ROTTEN_GROUND = register(blockWithItem("rotten_ground", new RottenGroundBlock(Settings.copy(MUD).mapColor(MapColor.PALE_PURPLE).sounds(BlockSoundGroup.HONEY).velocityMultiplier(0.775F).jumpVelocityMultiplier(0.9F)), DyeColor.LIME).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(TexturedModel.CUBE_ALL.upload(block, ctx.modelCollector)).put(VariantSettings.WEIGHT, 4), createModelVariant(SpectrumTexturedModels.cubeAll(b -> b, "_bony").upload(block, "_bony", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cubeAll(b -> b, "_boil").upload(block, "_boil", ctx.modelCollector)))));
	...
	public static final Block ASH = register(blockWithItem("ash", new AshBlock(ash(BlockSoundGroup.POWDER_SNOW)), DyeColor.GRAY).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(SpectrumTexturedModels.cubeAll(b -> b, "").upload(block, "", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cubeAll(b -> b, "2").upload(block, "2", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cubeAll(b -> b, "3").upload(block, "3", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cubeAll(b -> b, "4").upload(block, "4", ctx.modelCollector)))));
	public static final Block ASH_PILE = register(blockWithItem("ash_pile", new AshPileBlock(ash(BlockSoundGroup.POWDER_SNOW).replaceable().blockVision((state, world, pos) -> state.get(SnowBlock.LAYERS) >= 8).pistonBehavior(PistonBehavior.DESTROY)), DyeColor.LIGHT_GRAY).withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, block, "_height2")).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.LAYERS).registerVariants(height -> {
	...
	public static final Block VARIA_SPROUT = register(cutout(blockWithItem("varia_sprout", new AshFloraBlock(settings(MapColor.WHITE, BlockSoundGroup.NETHER_STEM, 0F).breakInstantly().luminance(state -> 11).offset(OffsetType.XZ).dynamicBounds().noCollision().postProcess(SpectrumBlocks::always).emissiveLighting(SpectrumBlocks::always)), DyeColor.WHITE)).withBlockItemModel(SpectrumModelProvider::registerBlockTexturedItemModel).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(SpectrumTexturedModels.cross(b -> b, "").upload(block, "", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_2").upload(block, "_2", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_3").upload(block, "_3", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_4").upload(block, "_4", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_5").upload(block, "_5", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_6").upload(block, "_6", ctx.modelCollector)))));
	...
	public static FungusBlock registerNoxshroom(String name, RegistryKey<ConfiguredFeature<?, ?>> feature, MapColor mapColor) {
		return register(cutout(blockWithItem(name, new FungusBlock(feature, SHIMMEL, settings(mapColor, BlockSoundGroup.FUNGUS, 0.0F).noCollision()), DyeColor.LIME)).withBlockItemModel(SpectrumModelProvider::registerBlockTexturedItemModel).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(SpectrumTexturedModels.cross(b -> b, "_type_1").upload(block, "_type_1", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_type_2").upload(block, "_type_2", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_type_3").upload(block, "_type_3", ctx.modelCollector)))));
	}
	...
	public static <T extends FlexLanternBlock> T registerNoxwoodLantern(String name, T flexLanternBlock, DyeColor color) {
		return register(cutout(blockWithItem(name, flexLanternBlock, color)).withItemModel((ctx, item) -> registerItemModel(ctx, item, "_item")).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.HANGING, DiagonalBlock.DIAGONAL, FlexLanternBlock.TALL).register((hanging, diagonal, tall) -> {
	...
	public static final Block SLATE_NOXWOOD_LANTERN = registerNoxwoodLantern("slate_noxwood_lantern", new FlexLanternBlock(AbstractBlock.Settings.copy(LANTERN).luminance(s -> 13).pistonBehavior(PistonBehavior.DESTROY)), DyeColor.LIME);
	...
	public static final Block EBONY_NOXWOOD_LANTERN = registerNoxwoodLantern("ebony_noxwood_lantern", new FlexLanternBlock(AbstractBlock.Settings.copy(LANTERN).luminance(s -> 13).pistonBehavior(PistonBehavior.DESTROY)), DyeColor.LIME);
	...
	public static final Block IVORY_NOXWOOD_LANTERN = registerNoxwoodLantern("ivory_noxwood_lantern", new FlexLanternBlock(AbstractBlock.Settings.copy(LANTERN).luminance(s -> 13).pistonBehavior(PistonBehavior.DESTROY)), DyeColor.LIME);
	...
	public static final Block CHESTNUT_NOXWOOD_LANTERN = registerNoxwoodLantern("chestnut_noxwood_lantern", new FlexLanternBlock(AbstractBlock.Settings.copy(LANTERN).luminance(s -> 13).pistonBehavior(PistonBehavior.DESTROY)), DyeColor.LIME);
	...
	public static final WeepingGalaSprigBlock WEEPING_GALA_SPRIG = register(cross(blockWithItem("weeping_gala_sprig", new WeepingGalaSprigBlock(copyWithMapColor(OAK_SAPLING, MapColor.BRIGHT_TEAL)), DyeColor.LIME)).withItemModel(SpectrumModelProvider::registerItemModel));
	...
	public static final Block WEEPING_GALA_BUTTON = register(blockWithItem("weeping_gala_button", createWoodenButtonBlock(GALA_BLOCK_SET_TYPE), DyeColor.LIME));
	...
	public static final Block WEEPING_GALA_LANTERN = register(translucent(blockWithItem("weeping_gala_lantern", new FlexLanternBlock(galaWood(MapColor.BROWN).luminance(state -> 13).nonOpaque().pistonBehavior(PistonBehavior.DESTROY)), DyeColor.LIME)).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.HANGING).register(false, BlockStateVariant.create()).register(true, BlockStateVariant.create().put(VariantSettings.X, VariantSettings.Rotation.R180))).coordinate(BlockStateVariantMap.create(DiagonalBlock.DIAGONAL, FlexLanternBlock.TALL).register((diagonal, tall) -> createModelVariant(SpectrumTexturedModels.baseTransLantern(diagonal, tall).upload(block, (diagonal ? "_diagonal" : "") + (tall ? "_tall" : "_small"), ctx.modelCollector))))).withItemModel((ctx, item) -> registerItemModel(ctx, item, "_item")));
	...
	public static SmallDragonjagBlock registerSmallDragonjagBlock(String name, Dragonjag.Variant variant) {
		return register(cutout(singleton(blockWithItem(name, new SmallDragonjagBlock(settings(variant.getMapColor(), BlockSoundGroup.GRASS, 1.0F), variant), DyeColor.LIME), SpectrumTexturedModels.doubleCross(b -> b, ""))).withBlockItemModel(SpectrumModelProvider::registerBlockTexturedItemModel));
	}
	...
	//Flora
	public static final Block ALOE = register(cutout(block("aloe", new AloeBlock(settings(MapColor.DARK_GREEN, BlockSoundGroup.GRASS, 1.0F).noCollision().ticksRandomly().nonOpaque()))).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_4).register(age -> createModelVariant(SpectrumTexturedModels.cross(b -> b, age.toString()).upload(block, age.toString(), ctx.modelCollector))))));
	public static final Block SAWBLADE_HOLLY_BUSH = register(cutout(block("sawblade_holly_bush", new SawbladeHollyBushBlock(settings(MapColor.TERRACOTTA_GREEN, BlockSoundGroup.GRASS, 0.0F).noCollision().ticksRandomly().nonOpaque().luminance(s -> s.get(SawbladeHollyBushBlock.AGE) == SawbladeHollyBushBlock.MAX_AGE ? 10 : 0)))).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_7).register(0, createModelVariant(SpectrumTexturedModels.cross(b -> b, "0").upload(block, "_stage0", ctx.modelCollector))).register(1, createModelVariant(SpectrumTexturedModels.cross(b -> b, "1").upload(block, "_stage1", ctx.modelCollector))).register(2, createModelVariant(block, "_stage1")).register(3, createModelVariant(SpectrumTexturedModels.cross(b -> b, "2").upload(block, "_stage2", ctx.modelCollector))).register(4, createModelVariant(block, "_stage2")).register(5, createModelVariant(block, "_stage2")).register(6, createModelVariant(block, "_stage2")).register(7, createModelVariant(SpectrumTexturedModels.cross(b -> b, "3").upload(block, "_stage3", ctx.modelCollector))))));
	public static final Block BRISTLE_SPROUTS = register(cutout(blockWithItem("bristle_sprouts", new BristleSproutsBlock(settings(MapColor.PALE_GREEN, BlockSoundGroup.GRASS, 0.0F).noCollision().nonOpaque().offset(OffsetType.XZ).replaceable()), DyeColor.LIME)).withBlockItemModel((ctx, block) -> registerBlockTexturedItemModel(ctx, block, "_1")).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(SpectrumTexturedModels.cross(b -> b, "_1").upload(block, "_1", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_2").upload(block, "_2", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_3").upload(block, "_3", ctx.modelCollector)), createModelVariant(SpectrumTexturedModels.cross(b -> b, "_4").upload(block, "_4", ctx.modelCollector)))));
	public static final Block DOOMBLOOM = register(cutout(block("doombloom", new DoomBloomBlock(SpectrumStatusEffects.STIFFNESS, 8, settings(MapColor.PALE_GREEN, BlockSoundGroup.GRASS, 0.0F).ticksRandomly().noCollision().luminance((state) -> state.get(DoomBloomBlock.AGE) * 2).nonOpaque()))).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_4).register(age -> createModelVariant(SpectrumTexturedModels.cross(b -> b, age.toString()).upload(block, age.toString(), ctx.modelCollector))))));
	public static final Block SNAPPING_IVY = register(cutout(blockWithItem("snapping_ivy", new SnappingIvyBlock(settings(MapColor.PALE_GREEN, BlockSoundGroup.GRASS, 3.0F).noCollision().nonOpaque()), DyeColor.RED)).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(createBooleanModelMap(SnappingIvyBlock.SNAPPED, ModelIds.getBlockSubModelId(block, "_snapped"), ModelIds.getBlockModelId(block))).coordinate(BlockStateVariantMap.create(Properties.HORIZONTAL_AXIS).register(Direction.Axis.X, BlockStateVariant.create()).register(Direction.Axis.Z, BlockStateVariant.create().put(VariantSettings.Y, VariantSettings.Rotation.R90)))));
	...
	public static final Block HUMMINGSTONE = register(translucent(blockWithItem("hummingstone", new HummingstoneBlock(Settings.copy(HUMMINGSTONE_GLASS).luminance((state) -> 14)), DyeColor.LIGHT_BLUE)).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(createBooleanModelMap(HummingstoneBlock.HUMMING, SpectrumTexturedModels.cubeAll(b -> b, "_humming").upload(block, "_humming", ctx.modelCollector), TexturedModel.CUBE_ALL.upload(block, ctx.modelCollector)))));
	public static final Block WAXED_HUMMINGSTONE = register(translucent(parented(blockWithItem("waxed_hummingstone", new TransparentBlock(Settings.copy(HUMMINGSTONE)), DyeColor.LIGHT_BLUE), b -> HUMMINGSTONE)));
	
	public static final Block MOSS_BALL = register(cutout(blockWithItem("moss_ball", new MossBallBlock(settings(MapColor.DARK_GREEN, BlockSoundGroup.WET_GRASS, 1F).noCollision().nonOpaque().offset(OffsetType.XYZ)), DyeColor.GREEN)).withBlockModel((ctx, block) -> {
	...
	public static final Block RESPLENDENT_BED = register(cutout(blockWithItem("resplendent_bed", new SpectrumBedBlock(DyeColor.RED, Settings.copy(RED_BED)), IS.of(1, Rarity.UNCOMMON), DyeColor.YELLOW)).withPredefinedItemModel().withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(createSouthDefaultHorizontalFacingVariantMap()).coordinate(BlockStateVariantMap.create(BedBlock.PART).register(BedPart.HEAD, createModelVariant(block, "_head")).register(BedPart.FOOT, createModelVariant(block, "_foot")))));
	...
	public static final Block JADE_VINES = register(cutout(block("jade_vines", new JadeVinePlantBlock(jadeVine().luminance((state) -> state.get(JadeVinePlantBlock.AGE) == 0 ? 0 : 5)))).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_7, JadeVinePlantBlock.PART).register((age, part) -> {
		...
		if (age == 0) return createModelVariant(model.upload(block, suffix, SpectrumTextureMaps.flowerParticle(SpectrumTextures.JADE_VINE_PLANT_RIPE, SpectrumTextures.JADE_VINE_PLANT_RIPE_BREAKING), ctx.modelCollector));
	...
	public static final Block NEPHRITE_BLOSSOM_STEM = register(cutout(blockWithItem("nephrite_blossom_stem", new NephriteBlossomStemBlock(settings(MapColor.PINK, BlockSoundGroup.WOOL, 2.0F).nonOpaque().noCollision()), DyeColor.PINK)).withBlockItemModel((ctx, block) -> registerBlockTexturedItemModel(ctx, block, "_bottom")).withBlockModel((ctx, block) -> {
		...
		return MultipartBlockStateSupplier.create(block).with(When.create().set(NephriteBlossomStemBlock.STEM_PART, StemComponent.STEM), createModelVariant(bottom)).with(When.create().set(NephriteBlossomStemBlock.STEM_PART, StemComponent.STEMALT), createModelVariant(base)).with(When.create().set(NephriteBlossomStemBlock.STEM_PART, StemComponent.BASE), createModelVariant(top), createModelVariant(bottom));
	}));
	public static final Block NEPHRITE_BLOSSOM_LEAVES = register(cutout(blockWithItem("nephrite_blossom_leaves", new NephriteBlossomLeavesBlock(settings(MapColor.PINK, BlockSoundGroup.GRASS, 0.2F).nonOpaque().ticksRandomly().luminance(state -> 13)), DyeColor.PINK)).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_2).register(age -> {
		String suffix = age == 0 ? "" : age == 1 ? "_flowering" : "_fruiting";
		return createModelVariant(SpectrumTexturedModels.leaves(b -> b, suffix).upload(block, suffix, ctx.modelCollector));
	}))));
	public static final Block NEPHRITE_BLOSSOM_BULB = register(cross(blockWithItem("nephrite_blossom_bulb", new NephriteBlossomBulbBlock(AbstractBlock.Settings.copy(NEPHRITE_BLOSSOM_STEM)), IS.of(16), DyeColor.PINK)).withItemModel(SpectrumModelProvider::registerItemModel));
	...
	public static final Block JADEITE_LOTUS_STEM = register(cutout(blockWithItem("jadeite_lotus_stem", new JadeiteLotusStemBlock(settings(MapColor.BLACK, BlockSoundGroup.WOOL, 2.0F).nonOpaque().noCollision()), DyeColor.LIME)).withBlockItemModel((ctx, block) -> registerBlockTexturedItemModel(ctx, block, "_top")).withBlockModel((ctx, block) -> {
		Identifier bottom = SpectrumTexturedModels.cross(b -> b, "_bottom").upload(block, "_bottom", ctx.modelCollector);
		Identifier top = SpectrumTexturedModels.cross(b -> b, "_top").upload(block, "_top", ctx.modelCollector);
		Identifier base = ModelIds.getBlockSubModelId(block, "_base");
		return MultipartBlockStateSupplier.create(block).with(When.create().set(JadeiteLotusStemBlock.STEM_PART, StemComponent.STEM), createModelVariant(bottom)).with(When.create().set(JadeiteLotusStemBlock.STEM_PART, StemComponent.STEMALT), createModelVariant(top)).with(When.create().set(JadeiteLotusStemBlock.STEM_PART, StemComponent.BASE), createModelVariant(bottom)).with(When.create().set(JadeiteLotusStemBlock.STEM_PART, StemComponent.BASE).set(Properties.INVERTED, false), createModelVariant(base)).with(When.create().set(JadeiteLotusStemBlock.STEM_PART, StemComponent.BASE).set(Properties.INVERTED, true), createModelVariant(base).put(VariantSettings.X, VariantSettings.Rotation.R180));
	}));
	...
	public static final Block JADEITE_LOTUS_BULB = register(cross(blockWithItem("jadeite_lotus_bulb", new JadeiteLotusBulbBlock(AbstractBlock.Settings.copy(JADEITE_LOTUS_STEM).nonOpaque()), IS.of(16), DyeColor.LIME)).withItemModel(SpectrumModelProvider::registerItemModel));
	...
	public static final Block JADEITE_PETAL_CARPET = register(cutout(singleton(blockWithItem("jadeite_petal_carpet", new CarpetBlock(jadeite()), DyeColor.LIME), SpectrumTexturedModels.carpet(b -> JADEITE_PETAL_BLOCK, ""))));
	...
	public static final Block SHIMMERSTONE_ORE = register(simple(blockWithItem("shimmerstone_ore", new ShimmerstoneOreBlock(UniformIntProvider.create(2, 4), ore().ticksRandomly(), SpectrumAdvancements.REVEAL_SHIMMERSTONE, STONE.getDefaultState()), DyeColor.YELLOW)));
	public static final Block DEEPSLATE_SHIMMERSTONE_ORE = register(simple(blockWithItem("deepslate_shimmerstone_ore", new ShimmerstoneOreBlock(UniformIntProvider.create(2, 4), deepslateOre().ticksRandomly(), SpectrumAdvancements.REVEAL_SHIMMERSTONE, DEEPSLATE.getDefaultState()), DyeColor.YELLOW)));
	...
	public static final Block STRATINE_ORE = register(simple(blockWithItem("stratine_ore", new CloakedOreBlock(UniformIntProvider.create(3, 5), netherrackOre(), SpectrumAdvancements.REVEAL_STRATINE, NETHERRACK.getDefaultState()), block -> new FloatBlockItem(block, IS.of().fireproof(), -0.01F), DyeColor.RED)));
	public static final Block PALTAERIA_ORE = register(simple(blockWithItem("paltaeria_ore", new CloakedOreBlock(UniformIntProvider.create(2, 4), endstoneOre(), SpectrumAdvancements.REVEAL_PALTAERIA, END_STONE.getDefaultState()), block -> new FloatBlockItem(block, IS.of(), 0.01F), DyeColor.CYAN)));
	...
	public static final FloatBlock STRATINE_FLOATBLOCK = register(singleton(blockWithItem("stratine_floatblock", new FloatBlock(gravityBlock(MapColor.DARK_RED), -0.2F), block -> new FloatBlockItem(block, IS.of(), 0.02F), DyeColor.CYAN), TexturedModel.CUBE_COLUMN));
	...
	public static final Block FABRICATION_CHEST = register(cutout(defaultNorthHorizontalFacing(blockWithItem("fabrication_chest", new FabricationChestBlock(settings(MapColor.ORANGE, BlockSoundGroup.STONE, 4.0F, 4.0F).requiresTool().nonOpaque()), DyeColor.YELLOW), ModelIds::getBlockModelId)).withPredefinedItemModel());
	...
	public static final Block BLAZING_CRYSTAL = register(simple(blockWithItem("blazing_crystal", new Block(AbstractBlock.Settings.copy(GLOWSTONE).mapColor(MapColor.ORANGE)), IS.of().fireproof(), DyeColor.ORANGE)));
	
	public static final Block QUITOXIC_REEDS = register(cross(blockWithItem("quitoxic_reeds", new QuitoxicReedsBlock(settings(MapColor.CLEAR, BlockSoundGroup.GRASS, 0.0F).noCollision().offset(AbstractBlock.OffsetType.XYZ).ticksRandomly().luminance(state -> state.get(QuitoxicReedsBlock.LOGGED).getLuminance())), DyeColor.PURPLE)).withItemModel(SpectrumModelProvider::registerItemModel));
	public static final Block MERMAIDS_BRUSH = register(cutout(block("mermaids_brush", new MermaidsBrushBlock(settings(MapColor.CLEAR, BlockSoundGroup.WET_GRASS, 0.0F).noCollision().ticksRandomly().luminance(state -> state.get(MermaidsBrushBlock.LOGGED).getLuminance())))).withBlockModel((ctx, block) -> {
		...
		return VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_7).register(age -> createModelVariant(age < 3 ? none : age < 6 ? some : full)));
	}));
	public static final Block RADIATING_ENDER = register(simple(blockWithItem("radiating_ender", new RadiatingEnderBlock(AbstractBlock.Settings.copy(EMERALD_BLOCK).mapColor(MapColor.PURPLE)), DyeColor.PURPLE)));
	...
	public static final Block MEMORY = register(translucent(singleton(blockWithItem("memory", new MemoryBlock(settings(MapColor.CLEAR, BlockSoundGroup.AMETHYST_BLOCK, 0.0F).blockVision(SpectrumBlocks::never).nonOpaque().ticksRandomly()), block -> new MemoryItem(block, IS.of(Rarity.UNCOMMON)), DyeColor.LIGHT_GRAY), ModelIds::getBlockModelId)).withPredefinedItemModel());
	public static final Block CRACKED_END_PORTAL_FRAME = register(blockWithItem("cracked_end_portal_frame", new CrackedEndPortalFrameBlock(settings(MapColor.PALE_PURPLE, BlockSoundGroup.GLASS, -1.0F, 3600000.0F).instrument(NoteBlockInstrument.BASEDRUM).luminance((state) -> 1)), IS.of().fireproof(), DyeColor.PURPLE).withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, block, "_none")).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(CrackedEndPortalFrameBlock.FACING_VERTICAL).register(false, BlockStateVariant.create()).register(true, BlockStateVariant.create().put(VariantSettings.Y, VariantSettings.Rotation.R90))).coordinate(BlockStateVariantMap.create(CrackedEndPortalFrameBlock.EYE_TYPE).register(type -> createModelVariant(ModelIds.getBlockSubModelId(block, "_" + type.asString()))))));
	...
	public static final Block REDSTONE_TIMER = register(cutout(blockWithItem("redstone_timer", new RedstoneTimerBlock(AbstractBlock.Settings.copy(REPEATER)), DyeColor.RED)).withPredefinedItemModel().withBlockModel((ctx, block) -> {
	...
	public static final Block REDSTONE_CALCULATOR = register(cutout(blockWithItem("redstone_calculator", new RedstoneCalculatorBlock(AbstractBlock.Settings.copy(REPEATER)), DyeColor.RED)).withPredefinedItemModel().withBlockModel((ctx, block) -> {
		MultipartBlockStateSupplier multipart = MultipartBlockStateSupplier.create(block);
		for (Direction direction : Direction.Type.HORIZONTAL) {
			VariantSettings.Rotation rotation = getSouthDefaultRotation(direction);
	...
	public static final Block REDSTONE_TRANSCEIVER = register(cutout(blockWithItem("redstone_transceiver", new RedstoneTransceiverBlock(AbstractBlock.Settings.copy(REPEATER)), DyeColor.RED)).withPredefinedItemModel().withBlockModel((ctx, block) -> {
		MultipartBlockStateSupplier multipart = MultipartBlockStateSupplier.create(block);
	...
	public static final Block BLOCK_PLACER = register(blockWithItem("block_placer", new BlockPlacerBlock(AbstractBlock.Settings.copy(DISPENSER)), DyeColor.CYAN).withBlockModel((ctx, block) -> createVariantsSupplier(ctx, block, SpectrumTexturedModels.complexOrientable(b -> b, "_side", b -> b, "_top", b -> NOTCHED_POLISHED_CALCITE, "_top", b -> b, "_front", b -> b, "_back", b -> b, "_side")).coordinate(createUpNorthDefaultOrientationVariantMap())));
	public static final Block BLOCK_DETECTOR = register(blockWithItem("block_detector", new BlockDetectorBlock(AbstractBlock.Settings.copy(DISPENSER)), DyeColor.CYAN).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(createUpNorthDefaultOrientationVariantMap()).coordinate(createBooleanModelMap(Properties.TRIGGERED, SpectrumTexturedModels.complexOrientable(b -> b, "_side", b -> b, "_top", b -> NOTCHED_POLISHED_BASALT, "_top", b -> b, "_front", b -> b, "_back_active", b -> b, "_side").upload(block, "_active", ctx.modelCollector), SpectrumTexturedModels.complexOrientable(b -> b, "_side", b -> b, "_top", b -> NOTCHED_POLISHED_BASALT, "_top", b -> b, "_front", b -> b, "_back", b -> b, "_side").upload(block, ctx.modelCollector)))));
	public static final Block BLOCK_BREAKER = register(blockWithItem("block_breaker", new BlockBreakerBlock(AbstractBlock.Settings.copy(DISPENSER)), DyeColor.CYAN).withBlockModel((ctx, block) -> createVariantsSupplier(ctx, block, SpectrumTexturedModels.complexOrientable(b -> b, "_side", b -> b, "_top", b -> POLISHED_BONE_ASH_PILLAR, "_top", b -> b, "_front", b -> b, "_back", b -> b, "_side")).coordinate(createUpNorthDefaultOrientationVariantMap())));
	public static final EnderDropperBlock ENDER_DROPPER = register(orientable(blockWithItem("ender_dropper", new EnderDropperBlock(AbstractBlock.Settings.copy(DROPPER).mapColor(MapColor.GRAY).requiresTool().strength(15F, 60.0F)), DyeColor.PURPLE)));
	public static final Block ENDER_HOPPER = register(singleton(blockWithItem("ender_hopper", new EnderHopperBlock(AbstractBlock.Settings.copy(HOPPER).mapColor(MapColor.GRAY).requiresTool().strength(15F, 60.0F)), DyeColor.PURPLE), ModelIds::getBlockModelId).withItemModel(SpectrumModelProvider::registerItemModel));
	
	public static final Block OMINOUS_SAPLING = register(simplePlant(blockWithItem("ominous_sapling", new OminousSaplingBlock(AbstractBlock.Settings.copy(OAK_SAPLING)), block -> new OminousSaplingBlockItem(block, IS.of()), DyeColor.GREEN)));
	
	public static final Block SPIRIT_SALLOW_LEAVES = register(singleton(blockWithItem("spirit_sallow_leaves", new SpiritSallowLeavesBlock(AbstractBlock.Settings.copy(OAK_LEAVES).mapColor(MapColor.OFF_WHITE).luminance((state) -> 8)), DyeColor.GREEN), TexturedModel.LEAVES));
	public static final Block SPIRIT_SALLOW_LOG = register(log(blockWithItem("spirit_sallow_log", new PillarBlock(AbstractBlock.Settings.copy(OAK_WOOD).mapColor(MapColor.GRAY)), DyeColor.GREEN)));
	public static final Block SPIRIT_SALLOW_ROOTS = register(blockWithItem("spirit_sallow_roots", new PillarBlock(AbstractBlock.Settings.copy(OAK_WOOD).mapColor(MapColor.GRAY)), DyeColor.GREEN).withBlockModel((ctx, block) -> {
	...
	public static final Block SPIRIT_SALLOW_HEART = register(singleton(blockWithItem("spirit_sallow_heart", new Block(AbstractBlock.Settings.copy(OAK_WOOD).mapColor(MapColor.GRAY).luminance(s -> 11)), DyeColor.GREEN), SpectrumTexturedModels.cubeColumn(b -> b, "", b -> SPIRIT_SALLOW_LOG, "_top")));
	
	public static final Block SACRED_SOIL = register(blockWithItem("sacred_soil", new ExtraTickFarmlandBlock(AbstractBlock.Settings.copy(FARMLAND).mapColor(MapColor.LIGHT_BLUE_GRAY), DIRT.getDefaultState()), DyeColor.LIME).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.MOISTURE).register(moisture -> createModelVariant(block, moisture == 7 ? "_moist" : "")))));
	...
	public static final Block DEEPER_DOWN_PORTAL = register(block("deeper_down_portal", new DeeperDownPortalBlock(settings(MapColor.BLACK, BlockSoundGroup.INTENTIONALLY_EMPTY, -1.0F, 3600000.0F).pistonBehavior(PistonBehavior.BLOCK).luminance(state -> 8).dropsNothing())).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(createBooleanModelMap(DeeperDownPortalBlock.FACING_UP, ModelIds.getBlockSubModelId(block, "_up"), ModelIds.getBlockModelId(block)))));
	
	private static Settings upgrade() {
		return AbstractBlock.Settings.copy(POLISHED_BASALT).solid();
	}
	
	public static final Block UPGRADE_SPEED = register(parented(blockWithItem("upgrade_speed", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.SPEED, 1, DyeColor.MAGENTA), block -> new UpgradeBlockItem(block, IS.of(16), "upgrade_speed"), DyeColor.LIGHT_GRAY), b -> b));
	public static final Block UPGRADE_SPEED2 = register(parented(blockWithItem("upgrade_speed2", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.SPEED, 2, DyeColor.MAGENTA), block -> new UpgradeBlockItem(block, IS.of(16, Rarity.UNCOMMON), "upgrade_speed2"), DyeColor.LIGHT_GRAY), b -> UPGRADE_SPEED));
	public static final Block UPGRADE_SPEED3 = register(parented(blockWithItem("upgrade_speed3", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.SPEED, 8, DyeColor.MAGENTA), block -> new UpgradeBlockItem(block, IS.of(16, Rarity.RARE), "upgrade_speed3"), DyeColor.LIGHT_GRAY), b -> UPGRADE_SPEED));
	public static final Block UPGRADE_EFFICIENCY = register(parented(blockWithItem("upgrade_efficiency", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.EFFICIENCY, 1, DyeColor.YELLOW), block -> new UpgradeBlockItem(block, IS.of(16, Rarity.UNCOMMON), "upgrade_efficiency"), DyeColor.LIGHT_GRAY), b -> b));
	public static final Block UPGRADE_EFFICIENCY2 = register(parented(blockWithItem("upgrade_efficiency2", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.EFFICIENCY, 4, DyeColor.YELLOW), block -> new UpgradeBlockItem(block, IS.of(16, Rarity.RARE), "upgrade_efficiency2"), DyeColor.LIGHT_GRAY), b -> UPGRADE_EFFICIENCY));
	public static final Block UPGRADE_YIELD = register(parented(blockWithItem("upgrade_yield", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.YIELD, 1, DyeColor.CYAN), block -> new UpgradeBlockItem(block, IS.of(16, Rarity.UNCOMMON), "upgrade_yield"), DyeColor.LIGHT_GRAY), b -> b));
	public static final Block UPGRADE_YIELD2 = register(parented(blockWithItem("upgrade_yield2", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.YIELD, 4, DyeColor.CYAN), block -> new UpgradeBlockItem(block, IS.of(16, Rarity.RARE), "upgrade_yield2"), DyeColor.LIGHT_GRAY), b -> UPGRADE_YIELD));
	public static final Block UPGRADE_EXPERIENCE = register(parented(blockWithItem("upgrade_experience", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.EXPERIENCE, 1, DyeColor.PURPLE), block -> new UpgradeBlockItem(block, IS.of(16), "upgrade_experience"), DyeColor.LIGHT_GRAY), b -> b));
	public static final Block UPGRADE_EXPERIENCE2 = register(parented(blockWithItem("upgrade_experience2", new UpgradeBlock(upgrade(), Upgradeable.UpgradeType.EXPERIENCE, 4, DyeColor.PURPLE), block -> new UpgradeBlockItem(block, IS.of(16, Rarity.UNCOMMON), "upgrade_experience2"), DyeColor.LIGHT_GRAY), b -> UPGRADE_EXPERIENCE));
	
	public static final Block REDSTONE_SAND = register(simple(blockWithItem("redstone_sand", new RedstoneGravityBlock(AbstractBlock.Settings.copy(SAND).mapColor(MapColor.BRIGHT_RED)), DyeColor.RED)));
	public static final Block ENDER_GLASS = register(translucent(blockWithItem("ender_glass", new EnderGlassBlock(AbstractBlock.Settings.copy(GLASS).mapColor(MapColor.PURPLE).nonOpaque().solidBlock(SpectrumBlocks::never).allowsSpawning((state, world, pos, entityType) -> state.get(EnderGlassBlock.TRANSPARENCY_STATE) == EnderGlassBlock.TransparencyState.SOLID).suffocates((state, world, pos) -> state.get(EnderGlassBlock.TRANSPARENCY_STATE) == EnderGlassBlock.TransparencyState.SOLID).blockVision((state, world, pos) -> state.get(EnderGlassBlock.TRANSPARENCY_STATE) == EnderGlassBlock.TransparencyState.SOLID)), DyeColor.PURPLE)).withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, block, "_solid")).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(EnderGlassBlock.TRANSPARENCY_STATE).register(transparency -> createModelVariant(SpectrumTexturedModels.cubeAll(b -> b, "_" + transparency.asString()).upload(block, "_" + transparency.asString(), ctx.modelCollector))))));
	public static final Block CLOVER = register(singleton(blockWithItem("clover", new CloverBlock(AbstractBlock.Settings.copy(SHORT_GRASS).offset(AbstractBlock.OffsetType.XZ)), IS.of(), DyeColor.LIME), ModelIds::getBlockModelId).withItemModel(SpectrumModelProvider::registerItemModel));
	public static final Block FOUR_LEAF_CLOVER = register(singleton(blockWithItem("four_leaf_clover", new FourLeafCloverBlock(AbstractBlock.Settings.copy(SHORT_GRASS).offset(AbstractBlock.OffsetType.XZ)), block -> new FourLeafCloverItem(block, IS.of(), SpectrumAdvancements.REVEAL_FOUR_LEAF_CLOVER, CLOVER.asItem()), DyeColor.LIME), ModelIds::getBlockModelId).withItemModel(SpectrumModelProvider::registerItemModel));
	
	private static final UniformIntProvider gemOreExperienceProvider = UniformIntProvider.create(1, 4);
	public static final Block TOPAZ_ORE = register(simple(blockWithItem("topaz_ore", new GemstoneOreBlock(gemOreExperienceProvider, ore(), BuiltinGemstoneColor.CYAN, SpectrumAdvancements.COLLECT_TOPAZ, STONE.getDefaultState()), DyeColor.CYAN)));
	public static final Block AMETHYST_ORE = register(simple(blockWithItem("amethyst_ore", new GemstoneOreBlock(gemOreExperienceProvider, ore(), BuiltinGemstoneColor.MAGENTA, SpectrumAdvancements.COLLECT_AMETHYST, STONE.getDefaultState()), DyeColor.MAGENTA)));
	public static final Block CITRINE_ORE = register(simple(blockWithItem("citrine_ore", new GemstoneOreBlock(gemOreExperienceProvider, ore(), BuiltinGemstoneColor.YELLOW, SpectrumAdvancements.COLLECT_CITRINE, STONE.getDefaultState()), DyeColor.YELLOW)));
	public static final Block ONYX_ORE = register(simple(blockWithItem("onyx_ore", new GemstoneOreBlock(gemOreExperienceProvider, ore(), BuiltinGemstoneColor.BLACK, SpectrumAdvancements.CREATE_ONYX, STONE.getDefaultState()), DyeColor.BLACK)));
	public static final Block MOONSTONE_ORE = register(simple(blockWithItem("moonstone_ore", new GemstoneOreBlock(gemOreExperienceProvider, ore(), BuiltinGemstoneColor.WHITE, SpectrumAdvancements.COLLECT_MOONSTONE, STONE.getDefaultState()), DyeColor.WHITE)));
	
	public static final Block DEEPSLATE_TOPAZ_ORE = register(simple(blockWithItem("deepslate_topaz_ore", new GemstoneOreBlock(gemOreExperienceProvider, deepslateOre(), BuiltinGemstoneColor.CYAN, SpectrumAdvancements.COLLECT_TOPAZ, DEEPSLATE.getDefaultState()), DyeColor.CYAN)));
	public static final Block DEEPSLATE_AMETHYST_ORE = register(simple(blockWithItem("deepslate_amethyst_ore", new GemstoneOreBlock(gemOreExperienceProvider, deepslateOre(), BuiltinGemstoneColor.MAGENTA, SpectrumAdvancements.COLLECT_AMETHYST, DEEPSLATE.getDefaultState()), DyeColor.MAGENTA)));
	public static final Block DEEPSLATE_CITRINE_ORE = register(simple(blockWithItem("deepslate_citrine_ore", new GemstoneOreBlock(gemOreExperienceProvider, deepslateOre(), BuiltinGemstoneColor.YELLOW, SpectrumAdvancements.COLLECT_CITRINE, DEEPSLATE.getDefaultState()), DyeColor.YELLOW)));
	public static final Block DEEPSLATE_ONYX_ORE = register(simple(blockWithItem("deepslate_onyx_ore", new GemstoneOreBlock(gemOreExperienceProvider, deepslateOre(), BuiltinGemstoneColor.BLACK, SpectrumAdvancements.CREATE_ONYX, DEEPSLATE.getDefaultState()), DyeColor.BLACK)));
	public static final Block DEEPSLATE_MOONSTONE_ORE = register(simple(blockWithItem("deepslate_moonstone_ore", new GemstoneOreBlock(gemOreExperienceProvider, deepslateOre(), BuiltinGemstoneColor.WHITE, SpectrumAdvancements.COLLECT_MOONSTONE, DEEPSLATE.getDefaultState()), DyeColor.WHITE)));
	
	public static final Block BLACKSLAG_TOPAZ_ORE = register(singleton(blockWithItem("blackslag_topaz_ore", new GemstoneOreBlock(gemOreExperienceProvider, blackslagOre(), BuiltinGemstoneColor.CYAN, SpectrumAdvancements.COLLECT_TOPAZ, BLACKSLAG.getDefaultState()), DyeColor.CYAN), TexturedModel.END_FOR_TOP_CUBE_COLUMN));
	...
	public static final Block BLACKSLAG_ONYX_ORE = register(singleton(blockWithItem("blackslag_onyx_ore", new GemstoneOreBlock(gemOreExperienceProvider, blackslagOre(), BuiltinGemstoneColor.BLACK, SpectrumAdvancements.CREATE_ONYX, BLACKSLAG.getDefaultState()), DyeColor.BLACK), TexturedModel.END_FOR_TOP_CUBE_COLUMN));
...
//	public static final Block SPECTRAL_SHARD_STORAGE_BLOCK = register(simple(blockWithItem("spectral_shard_storage_block", new Block(gemStorageBlock(MapColor.OFF_WHITE, SpectrumBlockSoundGroups.SPECTRAL_BLOCK)), IS.of(Rarity.RARE), DyeColor.WHITE)));
	...
	public static final AmaranthBushelBlock AMARANTH_BUSHEL = register(cross(blockWithItem("amaranth_bushel", new AmaranthBushelBlock(SpectrumStatusEffects.NOURISHING, 8, settings(MapColor.CLEAR, BlockSoundGroup.CROP, 0.0F).noCollision()), DyeColor.RED)).withItemModel(SpectrumModelProvider::registerItemModel));
	...
	public static final ResonantLilyBlock RESONANT_LILY = register(simplePlant(blockWithItem("resonant_lily", new ResonantLilyBlock(StatusEffects.REGENERATION, 5, AbstractBlock.Settings.copy(POPPY).mapColor(MapColor.WHITE)), DyeColor.GREEN)));
	...
	public static final BloodOrchidBlock BLOOD_ORCHID = register(cutout(blockWithItem("blood_orchid", new BloodOrchidBlock(SpectrumStatusEffects.FRENZY, 10, AbstractBlock.Settings.copy(POPPY).offset(AbstractBlock.OffsetType.NONE).ticksRandomly()), DyeColor.RED)).withBlockItemModel((ctx, block) -> registerBlockTexturedItemModel(ctx, block, "5")).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(BloodOrchidBlock.AGE).register(stage -> createModelVariant(SpectrumTexturedModels.cross(b -> b, stage.toString()).upload(block, stage.toString(), ctx.modelCollector))))));
	...
	public static ColoredLightBlock registerColoredLightBlock(String name, DyeColor color) {
		return register(translucent(blockWithItem(name, new ColoredLightBlock(AbstractBlock.Settings.copy(REDSTONE_LAMP).mapColor(color.getMapColor()), color), color)).withBlockModel((ctx, block) -> {
	...
	public static ColoredSporeBlossomBlock registerColoredSporeBlossomBlock(String name, DyeColor color) {
		return register(cutout(singleton(blockWithItem(name, new ColoredSporeBlossomBlock(AbstractBlock.Settings.copy(SPORE_BLOSSOM).mapColor(color.getMapColor()), color), color), TexturedModel.makeFactory(b -> SpectrumTextureMaps.flowerParticle(b, "", b, ""), SpectrumModels.SPORE_BLOSSOM))));
	}
	...
	public static ShimmerstoneLightBlock registerShimmerstoneLight(String name, BlockSoundGroup soundGroup, Supplier<Identifier> outerSupplier) {
		return register(blockWithItem(name, new ShimmerstoneLightBlock(settings(MapColor.CLEAR, soundGroup, 1.0F).nonOpaque().requiresTool().luminance(state -> 15)), DyeColor.YELLOW).withBlockModel((ctx, block) -> {
	...
	public static final Block SMALL_COAL_BUD = register(cluster(blockWithItem("small_coal_bud", new SpectrumClusterBlock(crystallarieumGrowable(COAL_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.BROWN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_IRON_BUD = register(cluster(blockWithItem("small_iron_bud", new SpectrumClusterBlock(crystallarieumGrowable(IRON_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.BROWN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_GOLD_BUD = register(cluster(blockWithItem("small_gold_bud", new SpectrumClusterBlock(crystallarieumGrowable(GOLD_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.BROWN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_DIAMOND_BUD = register(cluster(blockWithItem("small_diamond_bud", new SpectrumClusterBlock(crystallarieumGrowable(DIAMOND_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.CYAN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_EMERALD_BUD = register(cluster(blockWithItem("small_emerald_bud", new SpectrumClusterBlock(crystallarieumGrowable(EMERALD_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.CYAN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_REDSTONE_BUD = register(cluster(blockWithItem("small_redstone_bud", new SpectrumClusterBlock(crystallarieumGrowable(REDSTONE_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.RED), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_LAPIS_BUD = register(cluster(blockWithItem("small_lapis_bud", new SpectrumClusterBlock(crystallarieumGrowable(LAPIS_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.PURPLE), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_COPPER_BUD = register(cluster(blockWithItem("small_copper_bud", new SpectrumClusterBlock(crystallarieumGrowable(COPPER_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.BROWN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_QUARTZ_BUD = register(cluster(blockWithItem("small_quartz_bud", new SpectrumClusterBlock(crystallarieumGrowable(QUARTZ_BLOCK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.BROWN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_NETHERITE_SCRAP_BUD = register(cluster(blockWithItem("small_netherite_scrap_bud", new SpectrumClusterBlock(crystallarieumGrowable(ANCIENT_DEBRIS), SpectrumClusterBlock.GrowthStage.SMALL), IS.of().fireproof(), DyeColor.BROWN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_ECHO_BUD = register(cluster(blockWithItem("small_echo_bud", new SpectrumClusterBlock(crystallarieumGrowable(SCULK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.BROWN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_GLOWSTONE_BUD = register(cluster(blockWithItem("small_glowstone_bud", new SpectrumClusterBlock(crystallarieumGrowable(GLOWSTONE).luminance(state -> 4), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.YELLOW), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block SMALL_PRISMARINE_BUD = register(cluster(blockWithItem("small_prismarine_bud", new SpectrumClusterBlock(crystallarieumGrowable(SCULK), SpectrumClusterBlock.GrowthStage.SMALL), DyeColor.CYAN), SpectrumModels.CRYSTALLARIEUM_FARMABLE));
	...
	public static final Block PURE_COAL_BLOCK = register(simple(blockWithItem("pure_coal_block", new Block(AbstractBlock.Settings.copy(COAL_BLOCK)), DyeColor.BROWN)));
	public static final Block PURE_IRON_BLOCK = register(simple(blockWithItem("pure_iron_block", new Block(AbstractBlock.Settings.copy(IRON_BLOCK)), DyeColor.BROWN)));
	public static final Block PURE_GOLD_BLOCK = register(simple(blockWithItem("pure_gold_block", new Block(AbstractBlock.Settings.copy(GOLD_BLOCK)), DyeColor.BROWN)));
	public static final Block PURE_DIAMOND_BLOCK = register(simple(blockWithItem("pure_diamond_block", new Block(AbstractBlock.Settings.copy(DIAMOND_BLOCK)), DyeColor.CYAN)));
	public static final Block PURE_EMERALD_BLOCK = register(simple(blockWithItem("pure_emerald_block", new Block(AbstractBlock.Settings.copy(EMERALD_BLOCK)), DyeColor.CYAN)));
	public static final Block PURE_REDSTONE_BLOCK = register(simple(blockWithItem("pure_redstone_block", new PureRedstoneBlock(AbstractBlock.Settings.copy(REDSTONE_BLOCK)), DyeColor.RED)));
	public static final Block PURE_LAPIS_BLOCK = register(simple(blockWithItem("pure_lapis_block", new Block(AbstractBlock.Settings.copy(LAPIS_BLOCK)), DyeColor.PURPLE)));
	public static final Block PURE_COPPER_BLOCK = register(simple(blockWithItem("pure_copper_block", new Block(AbstractBlock.Settings.copy(COPPER_BLOCK)), DyeColor.BROWN)));
	public static final Block PURE_QUARTZ_BLOCK = register(simple(blockWithItem("pure_quartz_block", new Block(AbstractBlock.Settings.copy(QUARTZ_BLOCK)), DyeColor.BROWN)));
	public static final Block PURE_GLOWSTONE_BLOCK = register(simple(blockWithItem("pure_glowstone_block", new Block(AbstractBlock.Settings.copy(GLOWSTONE)), DyeColor.YELLOW)));
	public static final Block PURE_PRISMARINE_BLOCK = register(simple(blockWithItem("pure_prismarine_block", new Block(AbstractBlock.Settings.copy(PRISMARINE)), DyeColor.CYAN)));
	public static final Block PURE_NETHERITE_SCRAP_BLOCK = register(simple(blockWithItem("pure_netherite_scrap_block", new Block(AbstractBlock.Settings.copy(ANCIENT_DEBRIS)), IS.of().fireproof(), DyeColor.BROWN)));
	public static final Block PURE_ECHO_BLOCK = register(simple(blockWithItem("pure_echo_block", new Block(AbstractBlock.Settings.copy(DIAMOND_BLOCK)), DyeColor.BROWN)));
	...
	public static final Block PRESERVATION_CONTROLLER = register(cutout(singleton(blockWithItem("preservation_controller", new PreservationControllerBlock(preservationBlock().luminance(state -> 1).emissiveLighting(SpectrumBlocks::always).postProcess(SpectrumBlocks::always)), DyeColor.BLUE), ModelIds::getBlockModelId)).withPredefinedItemModel());
	...
	public static final Block INVISIBLE_WALL = register(translucent(singleton(blockWithItem("invisible_wall", new InvisibleWallBlock(preservationBlock().luminance(state -> 3).sounds(BlockSoundGroup.GLASS).nonOpaque().blockVision(SpectrumBlocks::never)), DyeColor.BLUE), SpectrumTexturedModels.particle(b -> GLASS, ""))).withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, ETHEREAL_PLATFORM)));
	...
	public static final Block PRESERVATION_STONE = register(blockWithItem("preservation_stone", new Block(preservationBlock()), DyeColor.BLUE).withBlockModel((ctx, block) -> {
	...
	public static final Block COURIER_STATUE = register(cutout(blockWithItem("courier_statue", new StatueBlock(preservationBlock()), DyeColor.BLUE)).withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, block, "_top")).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(createNorthDefaultHorizontalFacingVariantMap()).coordinate(BlockStateVariantMap.create(StatueBlock.HALF).register(DoubleBlockHalf.LOWER, createModelVariant(block, "_bottom")).register(DoubleBlockHalf.UPPER, createModelVariant(block, "_top")))));
	...
	public static final Block PRESERVATION_BLOCK_DETECTOR = register(blockWithItem("preservation_block_detector", new PreservationBlockDetectorBlock(preservationBlock()), DyeColor.BLUE).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block, createModelVariant(SpectrumTexturedModels.complexOrientable(b -> b, "_side", b -> b, "_top", b -> PRESERVATION_STONE, "_top_generic", b -> b, "_front", b -> b, "_back", b -> b, "_side").upload(block, ctx.modelCollector))).coordinate(createNorthDefaultFacingVariantMap())));
	...
	public static final Block STARDUST_BLOCK = register(simple(blockWithItem("stardust_block", new ColoredFallingBlock(new ColorCode(DyeColor.PURPLE.getFireworkColor()), AbstractBlock.Settings.copy(SAND).mapColor(MapColor.PURPLE)), IS.of(1, Rarity.UNCOMMON), DyeColor.BLACK)));
	
	public static final Block INCANDESCENT_AMALGAM = register(cutout(singleton(blockWithItem("incandescent_amalgam", new IncandescentAmalgamBlock(AbstractBlock.Settings.create().breakInstantly().nonOpaque()), block -> new IncandescentAmalgamItem(block, IS.of(16).food(SpectrumFoodComponents.INCANDESCENT_AMALGAM)), DyeColor.RED), ModelIds::getBlockModelId)).withBlockItemModel(SpectrumModelProvider::registerBlockTexturedItemModel));
	...
	public static final Block AXOLOTL_IDOL = register(idol(blockWithItem("axolotl_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.AXOLOTL_IDOL), ParticleTypes.HEART, StatusEffects.REGENERATION, 0, 100), DyeColor.PINK))); // heals 2 hp / 1 hear
	public static final Block BAT_IDOL = register(idol(blockWithItem("bat_idol", new AoEStatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.BAT_IDOL), ParticleTypes.INSTANT_EFFECT, StatusEffects.GLOWING, 0, 200, 8), DyeColor.PINK)));
	public static final Block BEE_IDOL = register(idol(blockWithItem("bee_idol", new BonemealingIdolBlock(idol(SpectrumBlockSoundGroups.BEE_IDOL), ParticleTypes.DRIPPING_HONEY), DyeColor.PINK)));
	public static final Block BLAZE_IDOL = register(idol(blockWithItem("blaze_idol", new FirestarterIdolBlock(idol(SpectrumBlockSoundGroups.BLAZE_IDOL), ParticleTypes.FLAME), DyeColor.PINK)));
	public static final Block CAT_IDOL = register(idol(blockWithItem("cat_idol", new FallDamageNegatingIdolBlock(idol(SpectrumBlockSoundGroups.CAT_IDOL), ParticleTypes.ENCHANTED_HIT), DyeColor.PINK)));
	public static final Block CHICKEN_IDOL = register(idol(blockWithItem("chicken_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.CHICKEN_IDOL), ParticleTypes.ENCHANTED_HIT, StatusEffects.SLOW_FALLING, 0, 100), DyeColor.PINK)));
	public static final Block COW_IDOL = register(idol(blockWithItem("cow_idol", new MilkingIdolBlock(idol(SpectrumBlockSoundGroups.COW_IDOL), ParticleTypes.ENCHANTED_HIT, 6), DyeColor.PINK)));
	public static final Block CREEPER_IDOL = register(idol(blockWithItem("creeper_idol", new ExplosionIdolBlock(idol(SpectrumBlockSoundGroups.CREEPER_IDOL), ParticleTypes.EXPLOSION, 3, false, Explosion.DestructionType.DESTROY), DyeColor.PINK)));
	...
	public static final Block ENDERMAN_IDOL = register(idol(blockWithItem("enderman_idol", new RandomTeleportingIdolBlock(idol(SpectrumBlockSoundGroups.ENDERMAN_IDOL), ParticleTypes.REVERSE_PORTAL, 16, 16), DyeColor.PINK)));
	public static final Block ENDERMITE_IDOL = register(idol(blockWithItem("endermite_idol", new LineTeleportingIdolBlock(idol(SpectrumBlockSoundGroups.ENDERMITE_IDOL), ParticleTypes.REVERSE_PORTAL, 16), DyeColor.PINK)));
	...
	public static final Block FISH_IDOL = register(idol(blockWithItem("fish_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.FISH_IDOL), ParticleTypes.SPLASH, StatusEffects.WATER_BREATHING, 0, 200), DyeColor.PINK)));
	public static final Block FOX_IDOL = register(idol(blockWithItem("fox_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.FOX_IDOL), ParticleTypes.ENCHANTED_HIT, StatusEffects.HASTE, 0, 200), DyeColor.PINK)));
	...
	public static final Block GLOW_SQUID_IDOL = register(idol(blockWithItem("glow_squid_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.GLOW_SQUID_IDOL), ParticleTypes.GLOW_SQUID_INK, StatusEffects.GLOWING, 0, 200), DyeColor.PINK)));
	public static final Block GOAT_IDOL = register(idol(blockWithItem("goat_idol", new KnockbackIdolBlock(idol(SpectrumBlockSoundGroups.GOAT_IDOL), ParticleTypes.ENCHANTED_HIT, 5.0F, 0.5F), DyeColor.PINK))); // knocks mostly sideways
	public static final Block GUARDIAN_IDOL = register(idol(blockWithItem("guardian_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.GUARDIAN_IDOL), ParticleTypes.BUBBLE, StatusEffects.MINING_FATIGUE, 2, 200), DyeColor.PINK)));
	public static final Block HORSE_IDOL = register(idol(blockWithItem("horse_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.HORSE_IDOL), ParticleTypes.INSTANT_EFFECT, StatusEffects.STRENGTH, 0, 100), DyeColor.PINK)));
	public static final Block ILLUSIONER_IDOL = register(idol(blockWithItem("illusioner_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.ILLUSIONER_IDOL), ParticleTypes.ANGRY_VILLAGER, StatusEffects.INVISIBILITY, 0, 100), DyeColor.PINK)));
	public static final Block OCELOT_IDOL = register(idol(blockWithItem("ocelot_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.OCELOT_IDOL), ParticleTypes.INSTANT_EFFECT, StatusEffects.NIGHT_VISION, 0, 100), DyeColor.PINK)));
	public static final Block PARROT_IDOL = register(idol(blockWithItem("parrot_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.PARROT_IDOL), ParticleTypes.INSTANT_EFFECT, StatusEffects.ABSORPTION, 0, 100), DyeColor.PINK)));
	public static final Block PHANTOM_IDOL = register(idol(blockWithItem("phantom_idol", new InsomniaIdolBlock(idol(SpectrumBlockSoundGroups.PHANTOM_IDOL), ParticleTypes.POOF, 24000), DyeColor.PINK))); // +1 ingame day without sleep
	public static final Block PIG_IDOL = register(idol(blockWithItem("pig_idol", new FeedingIdolBlock(idol(SpectrumBlockSoundGroups.PIG_IDOL), ParticleTypes.INSTANT_EFFECT, 6), DyeColor.PINK)));
	public static final Block PIGLIN_IDOL = register(idol(blockWithItem("piglin_idol", new PiglinTradeIdolBlock(idol(SpectrumBlockSoundGroups.PIGLIN_IDOL), ParticleTypes.HEART), DyeColor.PINK)));
	public static final Block POLAR_BEAR_IDOL = register(idol(blockWithItem("polar_bear_idol", new FreezingIdolBlock(idol(SpectrumBlockSoundGroups.POLAR_BEAR_IDOL), ParticleTypes.SNOWFLAKE), DyeColor.PINK)));
	public static final Block PUFFERFISH_IDOL = register(idol(blockWithItem("pufferfish_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.PUFFERFISH_IDOL), ParticleTypes.SPLASH, StatusEffects.NAUSEA, 0, 200), DyeColor.PINK)));
	public static final Block RABBIT_IDOL = register(idol(blockWithItem("rabbit_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.RABBIT_IDOL), ParticleTypes.INSTANT_EFFECT, StatusEffects.JUMP_BOOST, 3, 100), DyeColor.PINK)));
	public static final Block SHEEP_IDOL = register(idol(blockWithItem("sheep_idol", new ShearingIdolBlock(idol(SpectrumBlockSoundGroups.SHEEP_IDOL), ParticleTypes.ENCHANTED_HIT, 6), DyeColor.PINK)));
	public static final Block SHULKER_IDOL = register(idol(blockWithItem("shulker_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.SHULKER_IDOL), ParticleTypes.END_ROD, StatusEffects.LEVITATION, 0, 100), DyeColor.PINK)));
	public static final Block SILVERFISH_IDOL = register(idol(blockWithItem("silverfish_idol", new SilverfishInsertingIdolBlock(idol(SpectrumBlockSoundGroups.SILVERFISH_IDOL), ParticleTypes.EXPLOSION), DyeColor.PINK)));
	...
	public static final Block SLIME_IDOL = register(idol(blockWithItem("slime_idol", new SlimeSizingIdolBlock(idol(SpectrumBlockSoundGroups.SLIME_IDOL), ParticleTypes.ITEM_SLIME, 6, 8), DyeColor.PINK)));
	...
	public static final Block SPIDER_IDOL = register(idol(blockWithItem("spider_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.SPIDER_IDOL), ParticleTypes.ENCHANTED_HIT, StatusEffects.POISON, 0, 100), DyeColor.PINK)));
	public static final Block SQUID_IDOL = register(idol(blockWithItem("squid_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.SQUID_IDOL), ParticleTypes.SQUID_INK, StatusEffects.BLINDNESS, 0, 200), DyeColor.PINK)));
	public static final Block STRAY_IDOL = register(idol(blockWithItem("stray_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.STRAY_IDOL), ParticleTypes.ENCHANTED_HIT, StatusEffects.SLOWNESS, 2, 100), DyeColor.PINK)));
	public static final Block STRIDER_IDOL = register(idol(blockWithItem("strider_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.STRIDER_IDOL), ParticleTypes.DRIPPING_LAVA, StatusEffects.FIRE_RESISTANCE, 0, 200), DyeColor.PINK)));
	public static final Block TURTLE_IDOL = register(idol(blockWithItem("turtle_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.TURTLE_IDOL), ParticleTypes.DRIPPING_WATER, StatusEffects.RESISTANCE, 1, 200), DyeColor.PINK)));
	public static final Block WITCH_IDOL = register(idol(blockWithItem("witch_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.WITCH_IDOL), ParticleTypes.ENCHANTED_HIT, StatusEffects.WEAKNESS, 0, 200), DyeColor.PINK)));
	public static final Block WITHER_IDOL = register(idol(blockWithItem("wither_idol", new ExplosionIdolBlock(idol(SpectrumBlockSoundGroups.WITHER_IDOL), ParticleTypes.EXPLOSION, 7.0F, true, Explosion.DestructionType.DESTROY), DyeColor.PINK)));
	public static final Block WITHER_SKELETON_IDOL = register(idol(blockWithItem("wither_skeleton_idol", new StatusEffectIdolBlock(idol(SpectrumBlockSoundGroups.WITHER_SKELETON_IDOL), ParticleTypes.ENCHANTED_HIT, StatusEffects.WITHER, 0, 100), DyeColor.PINK)));
	public static final Block ZOMBIE_IDOL = register(idol(blockWithItem("zombie_idol", new VillagerConvertingIdolBlock(idol(SpectrumBlockSoundGroups.ZOMBIE_IDOL), ParticleTypes.ENCHANTED_HIT), DyeColor.PINK)));
	...
	public static <T extends Block> BlockRegistrar<T> parented(BlockRegistrar<T> registrar, UnaryOperator<Block> parent) {
		return registrar.withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, parent.apply(block))).withBlockModel((ctx, block) -> createVariantsSupplier(block, ModelIds.getBlockModelId(parent.apply(block))));
	}
	...
	public static <T extends Block> BlockRegistrar<T> defaultWestHorizontalFacing(BlockRegistrar<T> registrar, Function<Block, Identifier> modelIdGetter) {
		return registrar.withBlockModel((ctx, block) -> createVariantsSupplier(block, modelIdGetter.apply(block)).coordinate(createWestDefaultHorizontalFacingVariantMap()));
	}
	...
	public static <T extends Block> BlockRegistrar<T> defaultEastHorizontalFacing(BlockRegistrar<T> registrar, Function<Block, Identifier> modelIdGetter) {
		return registrar.withBlockModel((ctx, block) -> createVariantsSupplier(block, modelIdGetter.apply(block)).coordinate(createEastDefaultHorizontalFacingVariantMap()));
	}
	
	public static <T extends Block> BlockRegistrar<T> cross(BlockRegistrar<T> registrar) {
		return cutout(registrar).withBlockModel((ctx, block) -> createVariantsSupplier(block, SpectrumTexturedModels.cross(b -> b, "").upload(block, ctx.modelCollector)));
	}
	
	public static <T extends Block> BlockRegistrar<T> simplePlant(BlockRegistrar<T> registrar) {
		return cross(registrar).withBlockItemModel(SpectrumModelProvider::registerBlockTexturedItemModel);
	}
	...
	public static <T extends Block> BlockRegistrar<T> snowy(BlockRegistrar<T> registrar, TexturedModel.Factory base, TexturedModel.Factory snowy) {
		return registrar.withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.SNOWY).register(false, createHorizontalRotationVariantList(base.upload(block, ctx.modelCollector))).register(true, createHorizontalRotationVariantList(snowy.upload(block, "_snow", ctx.modelCollector)))));
	}
	
	public static <T extends Block> BlockRegistrar<T> redstoneLamp(BlockRegistrar<T> registrar) {
		return registrar.withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, block, "_off")).withBlockModel((ctx, block) -> {
	...
	public static <T extends Block> BlockRegistrar<T> sugarStick(BlockRegistrar<T> registrar, UnaryOperator<Block> sugarBlock) {
		return registrar.withItemModel(SpectrumModelProvider::registerItemModel).withBlockModel((ctx, block) -> VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.AGE_2).register(age -> createModelVariant(SpectrumTexturedModels.sugarStick(age, sugarBlock).upload(block, age.toString(), ctx.modelCollector)))));
	}
	...
	public static <T extends Block> BlockRegistrar<T> orientable(BlockRegistrar<T> registrar) {
		return registrar.withBlockModel((ctx, block) -> {
			...
			return VariantsBlockStateSupplier.create(block).coordinate(BlockStateVariantMap.create(Properties.FACING).register(Direction.DOWN, createModelVariant(vertical).put(VariantSettings.X, VariantSettings.Rotation.R180)).register(Direction.UP, createModelVariant(vertical)).register(Direction.NORTH, createModelVariant(horizontal)).register(Direction.EAST, createModelVariant(horizontal).put(VariantSettings.Y, VariantSettings.Rotation.R90)).register(Direction.SOUTH, createModelVariant(horizontal).put(VariantSettings.Y, VariantSettings.Rotation.R180)).register(Direction.WEST, createModelVariant(horizontal).put(VariantSettings.Y, VariantSettings.Rotation.R270)));
		});
	}
	
	public static <T extends Block> BlockRegistrar<T> pylon(BlockRegistrar<T> registrar) {
		return registrar.withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, block, "_head")).withBlockModel((ctx, block) -> {
	...
	public static void register() {
		...
		for (SpectrumSkullType type : SpectrumSkullType.values()) {
			BlockRegistrar<SpectrumSkullBlock> registrar = block(type.asString() + "_head", new SpectrumSkullBlock(type, AbstractBlock.Settings.copy(SKELETON_SKULL).instrument(NoteBlockInstrument.CUSTOM_HEAD))).withBlockItemModel((ctx, block) -> registerParentedItemModel(ctx, block, SpectrumModels.SKULL_ITEM)).withBlockModel((ctx, block) -> createVariantsSupplier(block, SpectrumModels.MOB_HEAD));
			Block wallHead = register(block(type.asString() + "_wall_head", new SpectrumWallSkullBlock(type, AbstractBlock.Settings.copy(SKELETON_SKULL).dropsLike(registrar.block()))).withBlockModel((ctx, block) -> createVariantsSupplier(block, SpectrumModels.MOB_BLOCK)));
