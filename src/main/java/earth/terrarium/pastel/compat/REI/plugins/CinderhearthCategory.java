...
public class CinderhearthCategory extends GatedDisplayCategory<CinderhearthDisplay> {
    ...
    public void setupWidgets(
        Point startPoint, Rectangle bounds, List<Widget> widgets, @NotNull CinderhearthDisplay display) {
        widgets.add(Widgets.createSlot(new Point(startPoint.x - 6, startPoint.y + 2))
                           .markInput()
                           .entries(display.getInputEntries()
                                           .get(0))); // input slot
        widgets.add(Widgets.createBurningFire(new Point(startPoint.x - 6, startPoint.y + 1 + 20))
                           .animationDurationMS(10000));
        widgets.add(Widgets.createArrow(new Point(startPoint.x - 6 + 18, startPoint.y + 2 + 5))
        ...
        for (int i = 0; i < outputs.size(); i++) {
            ...
            widgets.add(Widgets.createSlot(point)
                               .disableBackground()
                               .markOutput()
                               .entries(EntryIngredients.of(outputStack)));
            if (chance < 1.0) {
                widgets.add(Widgets.createLabel(
                                       new Point(point.x - 2, point.y + 23),
                                       Component.literal((int) (chance * 100) + " %"))
                                   .leftAligned()
                                   .color(0x3f3f3f)
                                   .noShadow());
            }
        ...
        widgets.add(Widgets.createLabel(new Point(startPoint.x - 6, startPoint.y + 1 + 43), text)
                           .leftAligned()
                           .color(0x3f3f3f)
                           .noShadow());
    }
