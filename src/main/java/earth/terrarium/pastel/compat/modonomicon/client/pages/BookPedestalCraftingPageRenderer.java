...
public class BookPedestalCraftingPageRenderer
    ...
    private static final ResourceLocation BACKGROUND_TEXTURE1 = PastelCommon.locate(
        "textures/gui/modonomicon/pedestal_crafting1.png");
    private static final ResourceLocation BACKGROUND_TEXTURE2 = PastelCommon.locate(
        "textures/gui/modonomicon/pedestal_crafting2.png");
    private static final ResourceLocation BACKGROUND_TEXTURE3 = PastelCommon.locate(
        "textures/gui/modonomicon/pedestal_crafting3.png");
    private static final ResourceLocation BACKGROUND_TEXTURE4 = PastelCommon.locate(
        "textures/gui/modonomicon/pedestal_crafting4.png");
    ...
    protected void drawRecipe(
        GuiGraphics drawContext, RecipeHolder<PedestalRecipe> recipeEntry, int recipeX, int recipeY, int mouseX,
        ...
        drawContext.blit(
            getBackgroundTextureForTier(recipe.getTier()), recipeX - 2, recipeY - 2, 0, 0, 106, 97, 128, 256);

        renderTitle(drawContext, recipeY, second);

        // the output
        parentScreen.renderItemStack(
            drawContext, recipeX + 78, recipeY + 22, mouseX, mouseY, recipe.getResultItem(world.registryAccess()));
        ...
        for (int i = 0; i < ingredients.size(); i++) {
            ModonomiconHelper.renderIngredientStack(
                drawContext, parentScreen, recipeX + (i % wrap) * 19 + 3, recipeY + (i / wrap) * 19 + 3, mouseX, mouseY,
                ingredients.get(i)
            );
        }
    }
    ...
    private void drawGemstonePowderSlots(
        GuiGraphics drawContext, PedestalRecipe recipe, GemstoneColor @NotNull [] colors, int startX, int recipeX,
        int recipeY, int mouseX, int mouseY
    ) {
        int h = 0;
        for (GemstoneColor color : colors) {
            ...
            if (amount > 0) {
                ...
                parentScreen.renderItemStack(
                    drawContext, recipeX + startX + h * 19, recipeY + 72, mouseX, mouseY, stack);
            }
            h++;
        }
    }
