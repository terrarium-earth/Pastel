...
public class WorkstaffItem extends MultiToolItem implements AreaMiningHandler, Preenchanted {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag type) {
        ...
        if (range > 0) {
            ...
            tooltip.add(
                Component.translatable("item.pastel.workstaff.tooltip.mining_range", displayedRange, displayedRange)
    ...
    public static void applyToggle(Player player, ItemStack stack, GUIToggle toggle) {
        ...
        switch (toggle) {
            ...
            case SELECT_SILK_TOUCH -> enchantAndRemoveOthers(
                player, stack, toggle.getTriggerText(), Enchantments.SILK_TOUCH);
            case SELECT_RESONANCE -> enchantAndRemoveOthers(
                player, stack, toggle.getTriggerText(), PastelEnchantments.RESONANCE);
            case ENABLE_RIGHT_CLICK_ACTIONS -> {
                stack.update(
                    PastelDataComponentTypes.WORKSTAFF, WorkstaffComponent.DEFAULT, comp ->
    ...
    private static void enchantAndRemoveOthers(
        Player player, ItemStack stack, Component message, ResourceKey<Enchantment> enchantment) {
        var registryLookup = player.level()
                                   .registryAccess();

        int existingLevel = Ench.getLevel(registryLookup, enchantment, stack);
        if (existingLevel > 0) {
            player.displayClientMessage(
                Component.translatable("item.pastel.workstaff.message.already_has_the_enchantment"), true);
            return;
        }
        ...
        if (enchantment == Enchantments.FORTUNE) {
            level = stack.getOrDefault(PastelDataComponentTypes.WORKSTAFF, WorkstaffComponent.DEFAULT)
                         .fortuneLevel();
        } else {
        ...
        var removeResult = Ench.removeEnchantments(
            registryLookup, newStack, Enchantments.SILK_TOUCH, PastelEnchantments.RESONANCE, Enchantments.FORTUNE);
        if (removeResult.getB() == 0) {
            if (player instanceof ServerPlayer serverPlayerEntity) {
                triggerUnenchantedWorkstaffAdvancement(serverPlayerEntity);
            }
        } else {
            var addResult = Ench.addOrUpgradeEnchantment(
                registryLookup, removeResult.getA(), enchantment, level, false, AdvancementHelper.hasAdvancement(
                    player,
                    PastelAdvancements.APPLY_CONFLICTING_ENCHANTMENTS
                )
            );
            if (addResult.getA()) {
                stack.set(
                    DataComponents.ENCHANTMENTS, addResult.getB()
                                                          .getEnchantments()
                );
                player.displayClientMessage(message, true);
            } else {
                player.displayClientMessage(
                    Component.translatable("item.pastel.workstaff.message.would_result_in_conflicting_enchantments"),
                    true
                );
            }
        }
    ...
    public boolean canPerformAction(ItemStack stack, ItemAbility itemAbility) {
        if (!stack.getOrDefault(PastelDataComponentTypes.WORKSTAFF, WorkstaffComponent.DEFAULT)
                  .canTill())
            return false;
