...
public record ParticleSpawnerConfiguration(
    ...
    public static final Codec<ParticleSpawnerConfiguration> CODEC = RecordCodecBuilder.create(i -> i.group(
                                                                                                        ...
                                                                                                        Codec.FLOAT.fieldOf("scale_variance")
    ...
    public static final StreamCodec<FriendlyByteBuf, ParticleSpawnerConfiguration> STREAM_CODEC
    ...
    private void spawnParticle(Level world, @NotNull BlockPos pos, RandomSource random) {
        float randomScale = scaleVariance == 0 ? scale : (float) (scale + scaleVariance -
                                                                  random.nextDouble() * scaleVariance * 2.0D);
        int randomLifetime = lifetimeVariance == 0 ? lifetimeTicks : (int) (lifetimeTicks + lifetimeVariance -
                                                                            random.nextDouble() * lifetimeVariance *
                                                                            2.0D);

        if (randomScale > 0 && randomLifetime > 0) {
            double randomOffsetX = sourcePositionVariance.x == 0 ? 0 : sourcePositionVariance.x -
                                                                       random.nextDouble() * sourcePositionVariance.x *
                                                                       2.0D;
            double randomOffsetY = sourcePositionVariance.y == 0 ? 0 : sourcePositionVariance.y -
                                                                       random.nextDouble() * sourcePositionVariance.y *
                                                                       2.0D;
            double randomOffsetZ = sourcePositionVariance.z == 0 ? 0 : sourcePositionVariance.z -
                                                                       random.nextDouble() * sourcePositionVariance.z *
                                                                       2.0D;

            double randomVelocityX = velocityVariance.x == 0 ? 0 : velocityVariance.x -
                                                                   random.nextDouble() * velocityVariance.x * 2.0D;
            double randomVelocityY = velocityVariance.y == 0 ? 0 : velocityVariance.y -
                                                                   random.nextDouble() * velocityVariance.y * 2.0D;
            double randomVelocityZ = velocityVariance.z == 0 ? 0 : velocityVariance.z -
                                                                   random.nextDouble() * velocityVariance.z * 2.0D;

            var rgbColor = CMYtoRGB(cmyColor);

            world.addParticle(
                new DynamicParticleEffect(
                    particleType, gravity, new Vector3f(rgbColor), randomScale, randomLifetime, collisions, glowing),
