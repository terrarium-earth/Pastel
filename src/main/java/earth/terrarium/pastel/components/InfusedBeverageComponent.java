...
public record InfusedBeverageComponent(String variant, int color) implements TooltipProvider {
    ...
    public static final Codec<InfusedBeverageComponent> CODEC = RecordCodecBuilder.create(i -> i.group(
                                                                                                    Codec.STRING.optionalFieldOf("variant", "unknown")
                                                                                                                .forGetter(InfusedBeverageComponent::variant),
                                                                                                    ColorHelper.CODEC.optionalFieldOf("color", 0xfff4c6cb)
    ...
    public void addToTooltip(Item.TooltipContext context, Consumer<Component> tooltip, TooltipFlag type) {
        tooltip.accept(Component.translatable("item.pastel.infused_beverage.tooltip.variant." + variant)
