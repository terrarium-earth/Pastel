...
public class PotionWorkshopReactingEmiRecipe extends EmiInfoRecipe {
    ...
    public boolean isUnlocked() {
        ...
        return DatabankUtils.hasAdvancement(client.player, PotionWorkshopRecipe.UNLOCK_IDENTIFIER) && DatabankUtils
            .hasAdvancement(client.player, recipe.advancementID().orElse(null));
    }
    ...
    public void addWidgets(WidgetHolder widgets) {
        if (!isUnlocked()) {
            widgets
                .addText(
                    PastelEmiRecipe.HIDDEN_LINE_1,
                    getDisplayWidth() / 2,
                    getDisplayHeight() / 2 - 8,
                    0x3f3f3f,
                    false
                )
            ...
            widgets
                .addText(
                    PastelEmiRecipe.HIDDEN_LINE_2,
                    getDisplayWidth() / 2,
                    getDisplayHeight() / 2 + 2,
                    0x3f3f3f,
                    false
                )
                .horizontalAlign(Alignment.CENTER);
