...
public class AnvilCrushingEmiRecipeGated extends GatedSpectrumEmiRecipe<AnvilCrushingRecipe> {
    ...
    public void addUnlockedWidgets(WidgetHolder widgets) {
        ...
        widgets.addSlot(EmiStack.of(Items.ANVIL), 21, 10)
               .drawBack(false);
        widgets.addTexture(WALL_TEXTURE, 22, 1, 16, 16, 16, 1); // falling stripes for anvil
        widgets.addTexture(EmiTexture.EMPTY_ARROW, 50, 16);
        widgets.addSlot(outputs.getFirst(), 90, 11)
               .large(true)
               .recipeContext(this);

        widgets.addText(
                   Component.translatable("container.pastel.rei.anvil_crushing.plus_xp", recipe.getExperience()),
                   width, 40, 0x3f3f3f, false
               )
               .horizontalAlign(Alignment.END);

        widgets.addText(getForceText(), width / 2, 54, 0x3f3f3f, false)
