...
public class ShootingStarSpawner implements CustomSpawner {
    ...
    public int tick(ServerLevel world, boolean spawnMonsters, boolean spawnAnimals) {
        ...
        for (
            Player playerEntity : world.getEntities(EntityType.PLAYER, Entity::isAlive)
        ) {
            if (!playerEntity.isSpectator() && DatabankUtils
                .hasAdvancement(playerEntity, PastelAdvancements.Milestones.UNLOCK_SHOOTING_STARS) && world
                    .getRandom()
    ...
    protected static float getShootingStarChanceWithMultiplier(@NotNull Player playerEntity) {
        int multiplier = 1;
        for (
            ItemStack handStack : playerEntity.getHandSlots()
        ) {
    ...
    public static void spawnShootingStar(ServerLevel serverWorld, @NotNull Player playerEntity) {
        ShootingStarEntity shootingStarEntity = new ShootingStarEntity(
            ...
            playerEntity
                .position()
                .z(),
            ShootingStar.Variant.getWeightedRandomType(serverWorld.getRandom()),
            false,
            3 + serverWorld.random.nextInt(5),
            false
        );
        shootingStarEntity
            .setDeltaMovement(
                serverWorld.random.nextDouble() * 0.2D - 0.1D,
                0.0D,
                serverWorld.random.nextDouble() * 0.2D - 0.1D
            );
