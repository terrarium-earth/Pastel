...
public class PotionWorkshopReactingEmiRecipe extends EmiInfoRecipe {
    ...
    public boolean isUnlocked() {
        ...
        return AdvancementHelper.hasAdvancement(client.player, PotionWorkshopRecipe.UNLOCK_IDENTIFIER) &&
               AdvancementHelper.hasAdvancement(
                   client.player, recipe.getRequiredAdvancementIdentifier()
                                        .orElse(null)
               );
    }
    ...
    public void addWidgets(WidgetHolder widgets) {
        if (!isUnlocked()) {
            widgets.addText(
                       PastelEmiRecipe.HIDDEN_LINE_1, getDisplayWidth() / 2, getDisplayHeight() / 2 - 8, 0x3f3f3f,
                       false)
                   .horizontalAlign(Alignment.CENTER);
            widgets.addText(
                       PastelEmiRecipe.HIDDEN_LINE_2, getDisplayWidth() / 2, getDisplayHeight() / 2 + 2, 0x3f3f3f,
                       false)
                   .horizontalAlign(Alignment.CENTER);
        } else {
