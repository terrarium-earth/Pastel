...
public class InkColors {
    ...
    private static final DeferredRegister<InkColor> REGISTER = DeferredRegister
        .create(
            PastelRegistryKeys.INK_COLOR,
            PastelCommon.MOD_ID
        );
    ...
    public static final InkColor LIGHT_BLUE = register(
        "light_blue",
        new InkColor(DyeColor.LIGHT_BLUE, LIGHT_BLUE_COLOR, BASE_ADVANCEMENT_ID)
    );

    public static final InkColor BLUE = register("blue", new InkColor(DyeColor.BLUE, BLUE_COLOR, BASE_ADVANCEMENT_ID));

    public static final InkColor PURPLE = register(
        "purple",
        new InkColor(DyeColor.PURPLE, PURPLE_COLOR, BASE_ADVANCEMENT_ID)
    );

    public static final InkColor MAGENTA = register(
        "magenta",
        new InkColor(DyeColor.MAGENTA, MAGENTA_COLOR, BASE_ADVANCEMENT_ID)
    );
    ...
    public static final InkColor ORANGE = register(
        "orange",
        new InkColor(DyeColor.ORANGE, ORANGE_COLOR, BASE_ADVANCEMENT_ID)
    );

    public static final InkColor YELLOW = register(
        "yellow",
        new InkColor(DyeColor.YELLOW, YELLOW_COLOR, BASE_ADVANCEMENT_ID)
    );

    public static final InkColor LIME = register("lime", new InkColor(DyeColor.LIME, LIME_COLOR, BASE_ADVANCEMENT_ID));

    public static final InkColor GREEN = register(
        "green",
        new InkColor(DyeColor.GREEN, GREEN_COLOR, BASE_ADVANCEMENT_ID)
    );

    public static final InkColor BROWN = register(
        "brown",
        new InkColor(DyeColor.BROWN, BROWN_COLOR, BLACK_ADVANCEMENT_ID)
    );

    public static final InkColor BLACK = register(
        "black",
        new InkColor(DyeColor.BLACK, BLACK_COLOR, BLACK_TEXT_COLOR, BLACK_ADVANCEMENT_ID)
    );

    public static final InkColor GRAY = register("gray", new InkColor(DyeColor.GRAY, GRAY_COLOR, WHITE_ADVANCEMENT_ID));

    public static final InkColor LIGHT_GRAY = register(
        "light_gray",
        new InkColor(DyeColor.LIGHT_GRAY, LIGHT_GRAY_COLOR, WHITE_ADVANCEMENT_ID)
    );

    public static final InkColor WHITE = register(
        "white",
        new InkColor(DyeColor.WHITE, WHITE_COLOR, WHITE_ADVANCEMENT_ID)
    );

    // in case an addon adds new colors
    // for places where we have to use a fixed size list, like GUIs with limited space
    public static final List<InkColor> BUILTIN_COLORS = List
        .of(
            InkColors.CYAN,
    ...
    public static List<InkColor> elementals() {
        return PastelRegistries.INK_COLOR
            .getTag(InkColorTags.ELEMENTAL_COLORS)
            .map(
                entries -> entries
                    .stream()
                    .map(Holder::value)
    ...
    public static List<InkColor> compounds() {
        return PastelRegistries.INK_COLOR
            .getTag(InkColorTags.COMPOUND_COLORS)
            .map(
                entries -> entries
                    .stream()
                    .map(Holder::value)
