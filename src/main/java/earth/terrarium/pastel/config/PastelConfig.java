package earth.terrarium.pastel.config;
...
public class PastelConfig implements ConfigData {
    ...
    @Comment(
        "Graphical options for pastel's dimension"
    )
    ...
    @Comment(
        "Affects how often the wind simulation updates - A lower number makes the simulation smoother, but increases " + "the performance impact significantly"
    )
    ...
    @Comment(
        "Adjusts the general brightness of the dimension (an increase of 1 is a lot, for reference)"
    )
    public float DimensionBrightnessMod = 1.0F;

    @Comment(
        "Mod Integration Packs to not load (in case of mod compat errors). Put the mod id of the mod with integration" + " pack conflict here"
    )
    ...
    @Comment(
        "Replaces the ornate models of pastel's transfer system with something simpler for increased visibility. " + "Great for technical players!"
    )
    ...
    @Comment(
        "The audio volume for Pastel's crafting blocks. Set to 0.0 to turn those sounds off completely."
    )
    ...
    @Comment(
        "If the Omni Accelerator should be able to have interactions in PvP that can drain the targets XP, modify " + "their equipment, ... (configured via the requires_omni_accelerator_pvp_enabled item tag)"
    )
    public boolean OmniAcceleratorPvP = false;

    @Comment(
        "The base range of the wire hook" + "Enchantments apply to this as a multiplier"
    )
