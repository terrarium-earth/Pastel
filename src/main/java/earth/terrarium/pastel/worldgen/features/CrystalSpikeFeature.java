...
public class CrystalSpikeFeature extends Feature<BlockStateFeatureConfig> {
    ...
    public boolean place(FeaturePlaceContext<BlockStateFeatureConfig> context) {
        ...
        var direction = new Vec3(0.0, 1.0, 0.0).xRot(Mth.PI / 4.0f)
                                               .yRot(level.getRandom()
                                                          .nextFloat() * 2 * Mth.PI);
        boolean up;
        ...
        for (int i = 0; i < SPIKE_LENGTH; i++) {
            ...
            placeLayer(
                centerPos, context.config()
                                  .blockState(), level, radius, PerlinNoise.create(level.getRandom(), -8, 1)
            );
        }
        return true;
    }
    ...
    private void placeLayer(BlockPos pos, BlockState state, WorldGenLevel level, int radius, PerlinNoise noise) {
        BlockPos.betweenClosed(pos.offset(radius, 0, radius), pos.offset(-radius, 0, -radius))
                .forEach((blockPos) -> {
                    if (blockPos.distSqr(pos) <= radius * radius && noise.getValue(pos.getX(), pos.getY(), pos.getZ()) <
                                                                    0.6) {
                        safeSetBlock(level, blockPos, state, (oldState) -> oldState.getBlock()
