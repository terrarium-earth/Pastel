...
public class PedestalScreen extends AbstractContainerScreen<PedestalScreenHandler> {
    ...
    protected void renderLabels(GuiGraphics drawContext, int mouseX, int mouseY) {
        ...
        drawContext.drawString(
            tr, this.playerInventoryTitle, inventoryX, intInventoryY, RenderHelper.GREEN_COLOR, false);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (mouseOverInformationIcon((int) mouseX, (int) mouseY)) {
            ...
            Multiblock multiblockToDisplay = PastelMultiblocks.get(
                maxPedestalRecipeTierForVariant.getStructureID(client.player));
            if (currentMultiBlock == multiblockToDisplay) {
                ...
                ModonomiconHelper.renderMultiblock(
                    multiblockToDisplay, maxPedestalRecipeTierForVariant.getStructureText(), this.menu.getBlockPos()
                                                                                                      .below(2),
                    Rotation.NONE
                );
            }
    ...
    public void render(GuiGraphics drawContext, int mouseX, int mouseY, float delta) {
        ...
        if (mouseOverInformationIcon(mouseX, mouseY)) {
            drawContext.renderTooltip(
                this.font, Component.translatable("multiblock.pastel.pedestal.upgrade_available"), mouseX, mouseY);
        } else {
    ...
    private boolean mouseOverInformationIcon(int mouseX, int mouseY) {
        return structureUpdateAvailable && mouseX > leftPos + informationIconX - 2 &&
               mouseX < leftPos + informationIconX + 10 && mouseY > topPos + informationIconY - 2 &&
               mouseY < topPos + informationIconY + 10;
    }
