...
public class DeepLightBlockEntityRenderer implements BlockEntityRenderer<DeepLightBlockEntity> {
    ...
    @Override
    public void render(
        DeepLightBlockEntity entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light,
        int overlay
    ) {
        if (Math.sqrt(Minecraft.getInstance().player.distanceToSqr(entity.getBlockPos()
                                                                         .getCenter())) > 6F)
            return;
        ...
        var interp = Mth.clamp(
            Math.sqrt(Minecraft.getInstance().player.distanceToSqr(entity.getBlockPos()
                                                                         .getCenter())) / 8F, 0, 1
        ) * 1.25F;
        ...
        for (int i = 0; i < text.size(); i++) {
            textRenderer.drawInBatch(
                text.get(i), 0, 14 * i + ((i == 0) ? -20 : 0), (alpha & 255) << 24 | 0xe9c4ff, false, matrices.last()
                                                                                                              .pose(),
                vertexConsumers, Font.DisplayMode.SEE_THROUGH, 0, LightTexture.FULL_BRIGHT
            );
        }
