...
public abstract class PastelFishingRodItem extends FishingRodItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player user, InteractionHand hand) {
        ...
        if (playerEntityAccessor.getSpectrumBobber() != null) {
            if (!world.isClientSide) {
                int damage = playerEntityAccessor.getSpectrumBobber()
                                                 .use(itemStack);
                itemStack.hurtAndBreak(damage, user, LivingEntity.getSlotForHand(hand));
            }

            world.playSound(
                null, user.getX(), user.getY(), user.getZ(), SoundEvents.FISHING_BOBBER_RETRIEVE, SoundSource.NEUTRAL,
                1.0F, 0.4F / (world.getRandom()
                                   .nextFloat() * 0.4F + 0.8F)
            );
            user.gameEvent(GameEvent.ITEM_INTERACT_FINISH);
        } else {
            world.playSound(
                null, user.getX(), user.getY(), user.getZ(), SoundEvents.FISHING_BOBBER_THROW, SoundSource.NEUTRAL,
                0.5F, 0.4F / (world.getRandom()
                                   .nextFloat() * 0.4F + 0.8F)
            );
            if (world instanceof ServerLevel serverWorld) {
                var drm = world.registryAccess();
                int luckBonus = EnchantmentHelper.getFishingLuckBonus(serverWorld, itemStack, user);
                int waitTimeReductionTicks = (int) (EnchantmentHelper.getFishingTimeReduction(
                    serverWorld, itemStack, user) * 20.0F);
                ...
                boolean inventoryInsertion = Ench.hasEnchantment(
                    drm, PastelEnchantments.INVENTORY_INSERTION, itemStack);
                boolean shouldSmeltDrops = shouldSmeltDrops(itemStack);
                spawnBobber(
                    user, world, luckBonus, waitTimeReductionTicks, exuberanceLevel, bigCatchLevel,
    ...
    public abstract void spawnBobber(
        Player user, Level world, int luckOfTheSeaLevel, int lureLevel, int exuberanceLevel, int bigCatchLevel,
    ...
    public boolean shouldSmeltDrops(ItemStack itemStack) {
        return EnchantmentHelper.hasTag(itemStack, PastelEnchantmentTags.SMELTS_MORE_LOOT) || GoFishCompat.hasDeepfry(
            itemStack);
    }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag type) {
        ...
        tooltip.add(Component.translatable("item.pastel.pastel_fishing_rods.tooltip")
