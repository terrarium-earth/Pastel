...
public class InkPoweredStatusEffectInstance {
    ...
    public static final Codec<InkPoweredStatusEffectInstance> CODEC = RecordCodecBuilder.create(i -> i.group(
                                                                                                          MobEffectInstance.CODEC.fieldOf("effect")
                                                                                                                                 .forGetter(c -> c.statusEffectInstance),
                                                                                                          InkCost.CODEC.fieldOf("ink_cost")
                                                                                                                       .forGetter(c -> c.cost),
                                                                                                          Codec.INT.optionalFieldOf("custom_color", -1)
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, InkPoweredStatusEffectInstance> STREAM_CODEC
    ...
    public InkPoweredStatusEffectInstance(
        MobEffectInstance statusEffectInstance, InkCost cost, int customColor, boolean unidentifiable,
        boolean incurable
    ) {
    ...
    public static List<InkPoweredStatusEffectInstance> getEffects(ItemStack stack) {
        return stack.getOrDefault(PastelDataComponentTypes.INK_POWERED, InkPoweredComponent.DEFAULT)
                    .effects();
    }
    ...
    public static void buildTooltip(
        List<Component> tooltip, List<InkPoweredStatusEffectInstance> effects, MutableComponent attributeModifierText,
        boolean showDuration, float tickRate
    ) {
        if (!effects.isEmpty()) {
            ...
            for (InkPoweredStatusEffectInstance entry : effects) {
                ...
                if (effect.getAmplifier() > 0) {
                    mutableText = Component.translatable(
                        "potion.withAmplifier", mutableText, Component.translatable(
                            "potion.potency." + effect.getAmplifier())
                    );
                }
                if (showDuration && effect.getDuration() > 20) {
                    mutableText = Component.translatable(
                        "potion.withDuration", mutableText, MobEffectUtil.formatDuration(effect, 1.0F, tickRate));
                }
                mutableText.withStyle(effect.getEffect()
                                            .value()
                                            .getCategory()
                                            .getTooltipFormatting());
                mutableText.append(
                    Component.translatable(
                                 "pastel.tooltip.ink_cost", Support.getShortenedNumberString(cost.cost()), cost.color()
                                                                                                               .getColoredInkName()
                             )
                ...
                effect.getEffect()
                      .value()
                      .createModifiers(
                          effect.getAmplifier(), (attribute, modifier) ->
            ...
            if (!attributeModifiers.isEmpty()) {
                ...
                for (var pair : attributeModifiers) {
                    var translatedAttribute = Component.translatable(pair.getA()
                                                                         .value()
                                                                         .getDescriptionId());
                    ...
                    double d;
                    if (mutableText.operation() != AttributeModifier.Operation.ADD_MULTIPLIED_BASE &&
                        mutableText.operation() != AttributeModifier.Operation.ADD_MULTIPLIED_TOTAL) {
                    ...
                    if (statusEffect > 0.0D) {
                        tooltip.add((Component.translatable(
                            "attribute.modifier.plus." + mutableText.operation()
                                                                    .id(),
                        ...
                        d *= -1.0D;
                        tooltip.add((Component.translatable(
                            "attribute.modifier.take." + mutableText.operation()
                                                                    .id(),
                            ItemAttributeModifiers.ATTRIBUTE_MODIFIER_FORMAT.format(d),
                            translatedAttribute
