...
public record InkCost(InkColor color, long cost) {
    ...
    public static final Codec<InkCost> CODEC = RecordCodecBuilder.create(i -> i.group(
                                                                                   InkColor.CODEC.fieldOf("color")
                                                                                                 .forGetter(InkCost::color),
                                                                                   Codec.LONG.fieldOf("cost")
                                                                                             .forGetter(InkCost::cost)
                                                                               )
                                                                               .apply(i, InkCost::new));

    public static final StreamCodec<ByteBuf, InkCost> STREAM_CODEC = StreamCodec.composite(
        InkColor.STREAM_CODEC, InkCost::color,
