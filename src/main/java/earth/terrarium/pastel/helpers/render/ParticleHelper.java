...
public class ParticleHelper {
    ...
    public static void playTriangulatedParticle(
        Level world, ParticleOptions particleEffect, int quantity, boolean triangular, Vec3 scale, double bonusYOffset,
        boolean solidSpawns, Vec3 position, Vec3 velocity
    ) {
        ...
        for (int i = 0; i < quantity; i++) {
            ...
            world.addParticle(
                particleEffect, position.x() + d, position.y() + e, position.z() + f, velocity.x(), velocity.y(),
                velocity.z()
            );
        }
    }
    ...
    public static void playParticleAroundBlockSides(
        Level world, ParticleOptions particleEffect, BlockPos position, Direction[] sides, int quantity,
        Vec3 velocity
    ) {
    ...
    public static void playParticleAroundBlockSides(
        Level world, ParticleOptions particleEffect, BlockPos position, List<Direction> sides, int quantity,
        Vec3 velocity
    ) {
        var random = world.getRandom();
        ...
        for (Direction direction : sides) {
            ...
            for (int i = 0; i < quantity; i++) {
                ...
                world.addParticle(
                    particleEffect, position.getX() + d, position.getY() + e, position.getZ() + f, velocity.x(),
