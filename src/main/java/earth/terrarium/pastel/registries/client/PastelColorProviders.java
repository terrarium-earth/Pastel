...
public class PastelColorProviders {
    ...
    private static final ItemColor THAT_ONE_VANILLA_ITEM_PROVIDER = (stack, tint) -> {
        BlockState blockstate = ((BlockItem) stack.getItem()).getBlock()
                                                             .defaultBlockState();
    ...
    public static void registerItems(RegisterColorHandlersEvent.Item event) {
        ...
        amaranthItem(event);
        event.register(
            YES_THERE_IS_ANOTHER_ITEM_ONE_TOO, PastelBlocks.CLOVER.get(), PastelBlocks.FOUR_LEAF_CLOVER.get());

        memoryItem(event, PastelBlocks.MEMORY.get());
        registerPotionFillables(
            event, PastelItems.LESSER_POTION_PENDANT.get(), PastelItems.GREATER_POTION_PENDANT.get(),
        ...
        event.register(
            (stack, tintIndex) -> {
                if (tintIndex == 1) {
                    ...
                    return FastColor.ARGB32.opaque(storage.getStoredColor()
                                                          .getColorInt());
                }
                return -1;
            }, items
        ...
        event.register(
            (stack, tintIndex) -> {
                if (tintIndex == 1) {
                    ...
                    if (!effects.isEmpty()) {
                        return FastColor.ARGB32.opaque(effects.getFirst()
                                                              .getColor());
                    }
                }
                return -1;
            ...
            (stack, tintIndex) -> {
                if (tintIndex > 0) {
                    ...
                    if (effects.size() > tintIndex - 1) {
                        return FastColor.ARGB32.opaque(effects.get(tintIndex - 1)
                                                              .getColor());
                    }
                }
                return -1;
        ...
        event.register(
            (stack, tintIndex) -> {
                ...
                return FastColor.ARGB32.opaque(
                    stack.getOrDefault(PastelDataComponentTypes.INFUSED_BEVERAGE, InfusedBeverageComponent.DEFAULT)
                         .color());
            }, brew
        );
    }
