    ...
    public static void registerBlockColorHandlers(RegisterColorHandlersEvent.Block event) {
        event.register(
            (state, level, pos, tintIndex) -> {
                if (tintIndex == 0 && state.getBlock() instanceof CushionBlock cushionBlock &&
                    cushionBlock.getColor() != null) {
                    return cushionBlock.getColor()
        ...
        event.register(
            (stack, tintIndex) -> {
                if (tintIndex == 0 && stack.getItem() instanceof BlockItem blockItem &&
                    blockItem.getBlock() instanceof CushionBlock cushionBlock && cushionBlock.getColor() != null)
                    return cushionBlock.getColor()
                                       .getTextureDiffuseColor();
                return 0xFFFFFFFF;
