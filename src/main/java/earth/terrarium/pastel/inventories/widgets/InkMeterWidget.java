...
public class InkMeterWidget implements Renderable, GuiEventListener, NarratableEntry {
    ...
    public InkMeterWidget(
        int x, int y, int height, Screen screen,
        InkStorageBlockEntity<IndividualCappedInkStorage> inkStorageBlockEntity
    ) {
        this.x = x;
        this.y = y;
        this.width = inkStorageBlockEntity.getEnergyStorage()
                                          .getSupportedColors()
                                          .size() * (WIDTH_PER_COLOR + SPACE_BETWEEN_COLORS) - SPACE_BETWEEN_COLORS;
        this.height = height;
        ...
        for (InkColor color : colors) {
            ...
            if (amount > 0) {
                ...
                RenderHelper.fillQuad(
                    drawContext.pose(), this.x + currentXOffset, startHeight - height, height, WIDTH_PER_COLOR,
                    color.getColorVec()
                );
            }
