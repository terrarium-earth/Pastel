...
public interface Weathering extends ChangeOverTimeBlock<Weathering.WeatheringLevel> {
    ...
    Supplier<BiMap<Block, Block>> WEATHERING_LEVEL_INCREASES = Suppliers.memoize(
    ...
    Supplier<BiMap<Block, Block>> WEATHERING_LEVEL_DECREASES = Suppliers.memoize(() -> WEATHERING_LEVEL_INCREASES.get()
                                                                                                                 .inverse());

    static Optional<Block> getDecreasedWeatheredBlock(Block block) {
        return Optional.ofNullable(WEATHERING_LEVEL_DECREASES.get()
                                                             .get(block));
    }
    ...
    static Block getUnaffectedWeatheredBlock(Block block) {
        ...
        for (
            Block block3 = WEATHERING_LEVEL_DECREASES.get()
                                                     .get(block); block3 != null;
            block3 = WEATHERING_LEVEL_DECREASES.get()
                                               .get(block3)
        ) {
            returnBlock = block3;
        }
    ...
    static Optional<Block> getIncreasedWeatheredBlock(Block block) {
        return Optional.ofNullable(WEATHERING_LEVEL_INCREASES.get()
                                                             .get(block));
    }
    ...
    default boolean shouldTryWeather(Level world, BlockPos pos) {
        ...
        if (world.isRaining() && world.getBiome(pos)
                                      .value()
                                      .getPrecipitationAt(pos) != Biome.Precipitation.NONE) {
            chance += 0.5;
        }
