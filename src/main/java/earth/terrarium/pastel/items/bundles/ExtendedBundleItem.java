...
public class ExtendedBundleItem extends BundleItem {
    ...
    // TODO: Currently, this isn't displayed properly by the tooltip component. If we make one, we can probably
    //  replace PresentTooltipComponent and BottomlessBundleTooltipComponent with it
    ...
    public int getBarWidth(ItemStack stack) {
        ...
        return Math
            .min(
                1 + Mth.mulAndTruncate(Fraction.getFraction(getStacks(stack).size(), getMaxStacks(stack)), 12),
                13
            );
    }
    ...
    public static Fraction getMaxOccupancy(ItemStack stack) {
        return stack
            .getOrDefault(PastelDataComponentTypes.EXTENDED_BUNDLE, ExtendedBundleComponent.DEFAULT)
            .maxOccupancy();
    }
    ...
    public static class ComponentBuilder extends BundleContents.Mutable {
        ...
        protected int findStackIndex(ItemStack stack) {
            if (stack.isStackable()) {
                ...
                for (
                    ...
                    if (ItemStack.isSameItemSameComponents(slotStack, stack) && slotStack.getCount() < slotStack
                        .getMaxStackSize()) {
                        return i;
                    }
                }
        ...
        protected int getMaxAmountToAdd(ItemStack stack) {
            ...
            var allowedByOccupancy = Math
                .max(
                    remainingOccupancy
                        .divideBy(itemOccupancy)
                        .intValue(),
                    0
                );
            ...
            for (
                int i = 0;
                i < Math.min(maxStacks, stacks.size());
                i++
            ) {
                var slotStack = stacks.get(i);
