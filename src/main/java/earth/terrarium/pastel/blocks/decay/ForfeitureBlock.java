...
public class ForfeitureBlock extends DecayBlock {
    ...
    public void setPlacedBy(
        ...
        if (!world.isClientSide) {
            world.playSound(null, pos, PastelSounds.FORFEITURE_PLACED, SoundSource.BLOCKS, 0.5F, 1.0F);
        } else {
            ...
            for (
                ...
                world
                    .addParticle(
                        ...
                        pos.getX() - 0.5 + random.nextFloat() * 2,
                        pos.getY() + random.nextFloat(),
                        pos.getZ() - 0.5 + random.nextFloat() * 2,
                        ((-1.0F + random.nextFloat() * 2.0F) / 12.0F),
                        0.05,
                        ((-1.0F + random.nextFloat() * 2.0F) / 12.0F)
                    );
    ...
    protected @Nullable BlockState getSpreadState(
        ...
        if (stateToSpreadTo.is(PastelBlockTags.FORFEITURE_SPECIAL_CONVERSIONS)) {
            ...
            if (world
                .dimension()
                .equals(Level.END) && Math.abs(stateToSpreadToPos.getX()) < 8 && Math
                    .abs(
                        stateToSpreadToPos.getZ()
                    ) < 8) {
                return null;
            }
