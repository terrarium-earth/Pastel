...
public class ShootingStarSpawner implements CustomSpawner {
    ...
    public int tick(ServerLevel world, boolean spawnMonsters, boolean spawnAnimals) {
        ...
        for (Player playerEntity : world.getEntities(EntityType.PLAYER, Entity::isAlive)) {
            if (!playerEntity.isSpectator()
                && AdvancementHelper.hasAdvancement(
                playerEntity, PastelItems.STAR_FRAGMENT.get()
                                                       .getCloakAdvancementIdentifier()
            )
                && world.getRandom()
                        .nextFloat() < getShootingStarChanceWithMultiplier(playerEntity)) {
    ...
    public static void spawnShootingStar(ServerLevel serverWorld, @NotNull Player playerEntity) {
        ShootingStarEntity shootingStarEntity = new ShootingStarEntity(
            serverWorld, playerEntity.position()
                                     .x(), playerEntity.position()
                                                       .y() + 200, playerEntity.position()
                                                                               .z(),
            ShootingStar.Variant.getWeightedRandomType(serverWorld.getRandom()), false,
            3 + serverWorld.random.nextInt(5), false
        );
        shootingStarEntity.setDeltaMovement(
            serverWorld.random.nextDouble() * 0.2D - 0.1D, 0.0D, serverWorld.random.nextDouble() * 0.2D - 0.1D);
