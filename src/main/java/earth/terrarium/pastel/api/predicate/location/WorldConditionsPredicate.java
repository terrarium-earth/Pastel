...
public record WorldConditionsPredicate(
    ...
    public WorldConditionsPredicate(
        ...
        this(
            moonPhase, timeOfDay, weather, command,
            new LocationPredicate(
                Optional.empty(), biomes, structures, dimension, smokey, light, block, fluid,
                canSeeSky
            )
        );
    }
    ...
    public static final Codec<WorldConditionsPredicate> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
                                                                                                                    MoonPhasePredicate.CODEC.optionalFieldOf("moon_phase")
                                                                                                                                            .forGetter(p -> p.moonPhase),
                                                                                                                    TimeOfDayPredicate.CODEC.optionalFieldOf("time_of_day")
                                                                                                                                            .forGetter(p -> p.timeOfDay),
                                                                                                                    WeatherPredicate.CODEC.optionalFieldOf("weather")
                                                                                                                                          .forGetter(p -> p.weather),
                                                                                                                    CommandPredicate.CODEC.optionalFieldOf("command")
                                                                                                                    ...
                                                                                                                    Codec.BOOL.optionalFieldOf("smokey")
                                                                                                                              .forGetter(p -> p.location.smokey()),
                                                                                                                    Codec.BOOL.optionalFieldOf("can_see_sky")
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, WorldConditionsPredicate> STREAM_CODEC
    ...
    public boolean test(ServerLevel world, BlockPos pos) {
        return location.matches(world, pos.getX(), pos.getY(), pos.getZ())
               && moonPhase.map(p -> p.test(world))
                           .orElse(true)
               && timeOfDay.map(p -> p.test(world))
                           .orElse(true)
               && weather.map(p -> p.test(world))
                         .orElse(true)
               && command.map(p -> p.test(world, pos))
                         .orElse(true);
    }
}
