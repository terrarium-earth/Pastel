...
public class MemoryItem extends BlockItem {
    ...
    public static ItemStack getMemoryForEntity(LivingEntity entity) {
        ...
        ItemStack stack = PastelBlocks.MEMORY.get()
                                             .asItem()
                                             .getDefaultInstance();
    ...
    public static ItemStack getForEntityType(EntityType<?> entityType, int ticksToManifest) {
        ...
        stack.set(
            PastelDataComponentTypes.MEMORY, new MemoryComponent.Builder(MemoryComponent.DEFAULT).ticksToManifest(
                                                                                                     ticksToManifest)
                                                                                                 .build()
        );

        CompoundTag entityCompound = new CompoundTag();
        entityCompound.putString(
    ...
    public static void setTicksToManifest(@NotNull ItemStack itemStack, int newTicksToManifest) {
        itemStack.update(
            PastelDataComponentTypes.MEMORY, MemoryComponent.DEFAULT, comp -> new MemoryComponent.Builder(
                comp).ticksToManifest(newTicksToManifest)
                     .build()
        );
    }
    ...
    public static void setSpawnAsAdult(@NotNull ItemStack itemStack, boolean spawnAsAdult) {
        itemStack.update(
            PastelDataComponentTypes.MEMORY, MemoryComponent.DEFAULT, comp -> new MemoryComponent.Builder(
                comp).spawnAsAdult(spawnAsAdult)
                     .build()
        );
    }
    ...
    public static void markAsBrokenPromise(ItemStack itemStack, boolean isBrokenPromise) {
        itemStack.update(
            PastelDataComponentTypes.MEMORY, MemoryComponent.DEFAULT, comp -> new MemoryComponent.Builder(
                comp).brokenPromise(isBrokenPromise)
                     .build()
        );
    }
        ...
        if (stack.has(PastelDataComponentTypes.MEMORY) && !isUnrecognizable(stack)) {
            ...
            if (entityType.isPresent()) {
                ...
                if (customColors.containsKey(type)) {
                    ...
                    return tintIndex == 0 ? customColors.get(type)
                                                        .getA() : customColors.get(type)
                                                                              .getB();
                } else {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag type) {
        ...
        getEntityType(stack).ifPresentOrElse(
            entityType -> {
                if (isUnrecognizable(stack)) {
                    tooltip.add(Component.translatable("item.pastel.memory.tooltip.unrecognizable_entity_type")
                    ...
                    if (isBrokenPromise) {
                        if (customName == null) {
                            ...
                            tooltip.add(Component.translatable("item.pastel.memory.tooltip.named_broken_promise")
                                                 .append(customName)
                        ...
                        if (customName == null) {
                            ...
                            tooltip.add(Component.translatable("item.pastel.memory.tooltip.named")
                                                 .append(customName)
                                                 .withStyle(ChatFormatting.WHITE, ChatFormatting.ITALIC));
                        }
                    }
                ...
                if (ticksToHatch <= 0) {
                    tooltip.add(Component.translatable("item.pastel.memory.tooltip.does_not_manifest")
                                         .withStyle(ChatFormatting.GRAY));
                } else if (ticksToHatch > 100) {
                    tooltip.add(Component.translatable("item.pastel.memory.tooltip.extra_long_time_to_manifest")
                                         .withStyle(ChatFormatting.GRAY));
                } else if (ticksToHatch > 20) {
                    tooltip.add(Component.translatable("item.pastel.memory.tooltip.long_time_to_manifest")
                                         .withStyle(ChatFormatting.GRAY));
                } else if (ticksToHatch > 5) {
                    tooltip.add(Component.translatable("item.pastel.memory.tooltip.medium_time_to_manifest")
                                         .withStyle(ChatFormatting.GRAY));
                } else {
                    tooltip.add(Component.translatable("item.pastel.memory.tooltip.short_time_to_manifest")
                                         .withStyle(ChatFormatting.GRAY));
                }
            }, () -> tooltip.add(Component.translatable("item.pastel.memory.tooltip.unset_entity_type")
                                          .withStyle(ChatFormatting.GRAY))
        );
    }
    ...
    public static void appendEntries(HolderLookup.Provider lookup, Output entries) {
        ...
        if (PastelCommon.getSidedServer() != null) {
            Item memoryItem = PastelBlocks.MEMORY.get()
                                                 .asItem();
            for (var recipe : PastelCommon.getSidedServer()
                                          .getRecipeManager()
