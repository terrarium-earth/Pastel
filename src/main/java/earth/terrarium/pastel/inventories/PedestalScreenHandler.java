...
public class PedestalScreenHandler extends RecipeBookMenu<RecipeInput, Recipe<RecipeInput>> {
    ...
    public record ScreenOpeningData(
        BlockPos pos, PedestalRecipeTier pedestalRecipeTier, PedestalRecipeTier maxRecipeTier
    ) {
    ...
    public PedestalScreenHandler(int syncId, Inventory playerInventory, ScreenOpeningData data) {
        this(
            syncId, playerInventory, playerInventory.player.level()
                                                           .getBlockEntity(
                                                               data.pos, PastelBlockEntities.PEDESTAL.get())
                                                           .orElseThrow(), new SimpleContainerData(2),
            data.pedestalRecipeTier, data.maxRecipeTier
        );
    }

    // serverside
    public PedestalScreenHandler(
        int syncId, Inventory playerInventory, PedestalBlockEntity blockEntity, ContainerData propertyDelegate,
        PedestalRecipeTier pedestalRecipeTier, PedestalRecipeTier maxRecipeTier
    ) {
        this(
            PastelScreenHandlerTypes.PEDESTAL, RecipeBookType.CRAFTING, syncId, playerInventory, blockEntity,
            propertyDelegate, pedestalRecipeTier, maxRecipeTier
        );
    }

    protected PedestalScreenHandler(
        MenuType<?> type, RecipeBookType recipeBookCategory, int i, Inventory playerInventory,
        PedestalBlockEntity blockEntity, ContainerData propertyDelegate, PedestalRecipeTier pedestalRecipeTier,
        PedestalRecipeTier maxRecipeTier
    ) {
        super(type, i);
    ...
    public boolean recipeMatches(RecipeHolder<Recipe<RecipeInput>> recipe) {
        return blockEntity != null && recipe.value()
                                            .matches(blockEntity.createRecipeInput(), world);
    }
        ...
        if (slot.hasItem()) {
            ...
            if (index < 15) {
                ...
                if (!this.moveItemStackTo(
                    clickedStack, PedestalBlockEntity.CRAFTING_TABLET_SLOT_ID,
                    PedestalBlockEntity.CRAFTING_TABLET_SLOT_ID + 1, false
                )) {
                    return ItemStack.EMPTY;
                }
            }
