...
public class CinderhearthScreenHandler extends AbstractContainerMenu {
    ...
    public CinderhearthScreenHandler(
        int syncId, Inventory playerInventory, CinderhearthBlockEntity blockEntity, ContainerData propertyDelegate) {
        ...
        this.addSlot(
            new ExperienceStorageItemSlot(
                blockEntity, CinderhearthBlockEntity.EXPERIENCE_STORAGE_ITEM_SLOT_ID, 38,
                52
            ));
        ...
        for (int i = 0; i < 4; i++) {
            this.addSlot(
                new Slot(blockEntity, CinderhearthBlockEntity.FIRST_OUTPUT_SLOT_ID + 4 + i, 62 + i * 18, 28 + 18));
        }
        ...
        if (slot.hasItem()) {
            ...
            if (index < CinderhearthBlockEntity.INVENTORY_SIZE) {
                if (!this.moveItemStackTo(
                    itemStack2, CinderhearthBlockEntity.INVENTORY_SIZE, this.slots.size(), true)) {
