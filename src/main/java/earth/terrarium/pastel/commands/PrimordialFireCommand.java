...
public class PrimordialFireCommand {
    ...
    public static void register(LiteralCommandNode<CommandSourceStack> root) {
        LiteralCommandNode<CommandSourceStack> primordialFire = Commands
            .literal("primordial_fire")
            .requires((source) -> source.hasPermission(2))
            .build();
        ArgumentCommandNode<CommandSourceStack, EntitySelector> targets = Commands
            .argument(
                "targets",
            ...
            .build();
        ArgumentCommandNode<CommandSourceStack, Integer> targetsDuration = Commands
            .argument(
                "duration",
    ...
    private static int execute(CommandSourceStack source, Collection<? extends Entity> targets, int ticks) {
        ...
        if (ticks > 0) {
            source
                .sendSuccess(
                    () -> Component.translatable("commands.pastel.primordial_fire.put_on.success", targets.size()),
                    false
                );
        } else {
            source
                .sendSuccess(
                    () -> Component.translatable("commands.pastel.primordial_fire.put_out.success", targets.size()),
                    false
                );
        }
