...
public class FabricationChestScreenHandler extends AbstractContainerMenu {
    ...
    protected FabricationChestScreenHandler(
        MenuType<?> type, int syncId, Inventory playerInventory, Container inventory) {
        super(type, syncId);
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot.hasItem()) {
            ...
            } else if (index > FabricationChestBlockEntity.INVENTORY_SIZE && clickedStackCopy.is(
                PastelItems.CRAFTING_TABLET.get())) {
                if (!this.moveItemStackTo(
                    clickedStack, FabricationChestBlockEntity.RECIPE_SLOTS[0], FabricationChestBlockEntity.RECIPE_SLOTS[
                                                                                   FabricationChestBlockEntity.RECIPE_SLOTS.length -
                                                                                   1] + 1, false
                )) {
                    return ItemStack.EMPTY;
                }
