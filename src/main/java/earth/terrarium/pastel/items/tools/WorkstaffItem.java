...
public class WorkstaffItem extends MultiToolItem implements AreaMiningHandler, Preenchanted {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag type) {
        ...
        if (range > 0) {
            ...
            tooltip
                .add(
                    Component
                        .translatable("item.pastel.workstaff.tooltip.mining_range", displayedRange, displayedRange)
    ...
    public MenuProvider createScreenHandlerFactory(ItemStack itemStack) {
        return new SimpleMenuProvider(
            (syncId, inventory, player) -> new WorkstaffScreenHandler(syncId, inventory, itemStack),
            ...
            case ENABLE_RIGHT_CLICK_ACTIONS -> {
                stack
                    .update(
                        ...
                        comp -> new WorkstaffComponent(true, comp.canShoot(), comp.fortuneLevel())
                    );
                player.displayClientMessage(toggle.getTriggerText(), true);
            }
            case DISABLE_RIGHT_CLICK_ACTIONS -> {
                stack
                    .update(
                        PastelDataComponentTypes.WORKSTAFF,
                        WorkstaffComponent.DEFAULT,
                        comp -> new WorkstaffComponent(false, comp.canShoot(), comp.fortuneLevel())
                    );
                player.displayClientMessage(toggle.getTriggerText(), true);
            }
            case ENABLE_PROJECTILES -> {
                stack
                    .update(
                        PastelDataComponentTypes.WORKSTAFF,
                        WorkstaffComponent.DEFAULT,
                        comp -> new WorkstaffComponent(comp.canTill(), true, comp.fortuneLevel())
                    );
                player.displayClientMessage(toggle.getTriggerText(), true);
            }
            case DISABLE_PROJECTILES -> {
                stack
                    .update(
                        PastelDataComponentTypes.WORKSTAFF,
                        WorkstaffComponent.DEFAULT,
                        comp -> new WorkstaffComponent(comp.canTill(), false, comp.fortuneLevel())
                    );
                player.displayClientMessage(toggle.getTriggerText(), true);
            }
        }
        ...
        if (existingLevel > 0) {
            player
                .displayClientMessage(
                    Component.translatable("item.pastel.workstaff.message.already_has_the_enchantment"),
                    true
                );
            return;
        }
        ...
        if (removeResult.getB() == 0) {
            ...
            if (level == 0) {
                stack
                    .update(
                        PastelDataComponentTypes.WORKSTAFF,
                        WorkstaffComponent.DEFAULT,
                        comp -> new WorkstaffComponent(comp.canTill(), comp.canShoot(), 4)
                    );
                level = 4;
            }
            ...
            if (addResult.getA()) {
                ...
                player
                    .displayClientMessage(
                        Component
                            .translatable("item.pastel.workstaff.message.would_result_in_conflicting_enchantments"),
                        true
                    );
            }
        }
