...
public class CraftingTabletScreenHandler extends RecipeBookMenu<RecipeInput, Recipe<RecipeInput>> {
    ...
    public CraftingTabletScreenHandler(
        ...
        ItemStack craftingTabletItemStack
    ) {
    ...
    protected void updateResult(AbstractContainerMenu handler, @NotNull Level world, Player player, CraftingTabletInventory inventory) {
        if (!world.isClientSide) {
            ...
            PedestalRecipeInput pedestalRecipeInput = PedestalRecipeInput
                .createWithFullGemstonePowder(inventory.getItems(), player);

            Optional<RecipeHolder<PedestalRecipe>> optionalPedestalCraftingRecipe = world
                .getRecipeManager()
                .getRecipeFor(PastelRecipeTypes.PEDESTAL, pedestalRecipeInput, world);
            if (optionalPedestalCraftingRecipe.isPresent()) {
                ...
                Optional<RecipeHolder<CraftingRecipe>> optionalCraftingRecipe = world
                    .getRecipeManager()
                    .getRecipeFor(RecipeType.CRAFTING, pedestalRecipeInput.getCraftingGridInput(), world);
                if (optionalCraftingRecipe.isPresent()) {
                    lockableCraftingResultSlot.unlock();

                    if (craftingResultInventory
                        .setRecipeUsed(world, serverPlayerEntity, optionalCraftingRecipe.get())) {
                        itemStack = optionalCraftingRecipe
                            .get()
                            .value()
                            .assemble(pedestalRecipeInput.getCraftingGridInput(), world.registryAccess());
                    }
    ...
    public boolean recipeMatches(RecipeHolder recipe) {
        PedestalRecipeInput pedestalRecipeInput = PedestalRecipeInput
            .createWithFullGemstonePowder(this.craftingInventory.getItems(), player);
                ...
                if (insertInventorySuccess && itemStack.isEmpty()) {
                    ...
                    playerEntity
                        ...
                        .playSound(
                            ...
                            SoundSource.PLAYERS,
                            0.2F,
                            ((playerEntity.getRandom().nextFloat() - playerEntity
                                .getRandom()
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot.hasItem()) {
            ...
            if (index == 14) {
                ...
                this.context
                    .execute((world, pos) -> clickedSlotStack.getItem().onCraftedBy(clickedSlotStack, world, player));
