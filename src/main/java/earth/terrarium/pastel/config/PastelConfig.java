...
public class PastelConfig implements ConfigData {
    ...
    @Comment(
        "Affects how often the wind simulation updates - A lower number makes the simulation smoother, but increases " +
        "the performance impact significantly")
    ...
    @Comment(
        "Mod Integration Packs to not load (in case of mod compat errors). Put the mod id of the mod with integration" +
        " pack conflict here")
    ...
    @Comment(
        "Replaces the ornate models of pastel's transfer system with something simpler for increased visibility. " +
    ...
    @Comment(
        "Worlds where shooting stars spawn for players. Shooting Stars will only spawn for players with sufficient " +
        "progress in the mod")
    ...
    @Comment(
        "When empty, enchantments that the player has not unlocked show up with a scattered name. You can use a " +
        "different name here")
    public String NameForUnrevealedEnchantments = "";

    @Comment(
        "Exuberance increases experience gained when killing mobs. With 25% bonus XP and 5 levels this would mean 2," +
        "25x XP on max level")
    public float ExuberanceBonusExperiencePercentPerLevel = 0.25F;

    @Comment(
        "In vanilla, crits are a flat 50 % damage bonus. Improved Critical increases this damage by additional 50 % " +
        "per level by default")
    ...
    @Comment(
        "The percentile a mobs armor/hand stacks are being dropped when hit with a Disarming enchanted weapon per the" +
        " enchantments level")
