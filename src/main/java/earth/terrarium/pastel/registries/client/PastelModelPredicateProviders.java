...
public class PastelModelPredicateProviders {
    ...
    public static void registerClient(FMLClientSetupEvent event) {
        ...
        registerOversizedItemPredicate(PastelItems.FLOWING_STAFF.get());
    ...
    private static void registerStampingItemPredicate(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("stamped"),
    ...
    private static void registerNullableInkColorPredicate(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("color"),
                (stack, clientWorld, entity, i) -> {
    ...
    private static void registerMysteriousLocketPredicates(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("socketed"),
                (stack, world, entity, i) -> stack.has(PastelDataComponentTypes.SOCKETED) ? 1.0F : 0.0F
            );
    }
    ...
    private static void registerMalachiteCrossbowPredicates(Item crossbowItem) {
        ItemProperties
            .register(
                ...
                (stack, world, user, i) -> user == null || CrossbowItem.isCharged(stack)
                    ? 0.0F
                    : (float) (stack.getUseDuration(user) - user.getUseItemRemainingTicks()) / (float) CrossbowItem
                        .getChargeDuration(
                            stack,
                            user
                        )
        ...
        ItemProperties
            .register(
                ...
                (stack, world, entity, i) -> entity != null && entity.isUsingItem() && entity
                    .getUseItem() == stack && !CrossbowItem.isCharged(stack)
                        ? 1.0F
                        : 0.0F
            );
        ...
        ItemProperties
            .register(
                ...
                (stack, world, entity, i) -> entity != null && CrossbowItem.isCharged(stack) ? 1.0F : 0.0F
            );

        ItemProperties
            .register(
                crossbowItem,
                ResourceLocation.parse("projectile"),
                (stack, world, entity, seed) -> {
                    if (stack == null) {
                        return 0F;
                    }
    ...
    private static void registerBidentThrowingItemPredicate(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("bident_throwing"),
                (stack, world, entity, seed) -> {
                    if (seed == 80085) {
                        return 1F;
                    }
    ...
    private static void registerPresentPredicates(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("variant"),
                (stack, world, entity, i) -> stack
    ...
    private static void registerBottomlessBundlePredicates(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("locked"),
                (stack, world, entity, i) -> BottomlessBundleItem.isLocked(stack) ? 1.0F : 0.0F
            );
        ...
        ItemProperties
            .register(
                ...
                (stack, world, entity, i) -> BottomlessBundleItem.getStoredAmount(stack) > 0 ? 1.0F : 0.0F
            );
    }
    ...
    private static void registerActivatableItemPredicate(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("activated"),
                (stack, world, entity, i) -> ActivatableItem.isActivated(stack) ? 1.0F : 0.0F
            );
    }
    ...
    private static void registerSlotReservingItem(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("reserved"),
                (stack, world, entity, i) -> SlotReservingItem.isReservingSlot(stack) ? 1.0F : 0.0F
            );
    }
    ...
    private static void registerOversizedItemPredicate(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("oversized"),
                (stack, world, entity, seed) -> seed == 817210941 || seed == 80085 ? 1.0F : 0.0F
            );
    }
    ...
    private static void registerBowPredicates(Item bowItem) {
        ItemProperties
            .register(
                bowItem,
                ResourceLocation.parse("pull"),
                (stack, world, entity, i) -> entity == null || entity.getUseItem() != stack
                    ? 0.0F
        ...
        ItemProperties
            .register(
                bowItem,
                ResourceLocation.parse("pulling"),
                (stack, world, entity, i) -> entity != null && entity.isUsingItem() && entity.getUseItem() == stack
                    ? 1.0F
                    : 0.0F
            );
    }
    ...
    private static void registerCrossbowPredicates(Item crossbowItem) {
        ItemProperties
            .register(
                crossbowItem,
                ResourceLocation.parse("pull"),
                (stack, world, entity, i) -> entity == null || CrossbowItem.isCharged(stack)
                    ? 0.0F
                    : (float) (stack.getUseDuration(entity) - entity.getUseItemRemainingTicks()) / (float) CrossbowItem
                        .getChargeDuration(
                            stack,
                            entity
                        )
    ...
    private static void registerPipeBombPredicates(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("armed"),
                (stack, world, entity, seed) -> PipeBombItem.isPrimed(stack) ? 1.0F : 0.0F
            );
    }
        ...
        ItemProperties
            .register(
                ...
                (stack, world, entity, i) -> {
                    ...
                    boolean isInOffhand = entity.getOffhandItem() == stack && !(entity
                        .getMainHandItem()
                        .getItem() instanceof PastelFishingRodItem);
                    return (isInMainHand || isInOffhand) && entity instanceof Player && ((PlayerEntityAccessor) entity)
                        .getSpectrumBobber() != null ? 1.0F : 0.0F;
                }
            );
    }
    ...
    private static void registerEnderSplicePredicates(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("bound"),
                (stack, world, entity, i) -> EnderSpliceItem.hasTeleportTarget(stack) ? 1.0F : 0.0F
            );
    }
    ...
    private static void registerAshenCircletPredicates(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("cooldown"),
                (stack, world, entity, i) -> world != null && AshenCircletItem.getCooldownTicks(stack, world) == 0
                    ? 0.0F
                    : 1.0F
            );
    }
    ...
    private static void registerInkFillStateItemPredicate(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("fill_state"),
                (stack, world, entity, i) -> {
                    SingleInkStorage storage = PastelItems.INK_FLASK
                        .get()
    ...
    private static void registerEnchantmentCanvasPredicates(Item item) {
        ItemProperties
            .register(
                item,
                ResourceLocation.parse("bound"),
                (stack, world, entity, i) -> stack.has(PastelDataComponentTypes.BOUND_ITEM) ? 1.0F : 0.0F
            );
    }
