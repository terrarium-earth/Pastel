...
public interface InkPowered {
            ...
            if (getUsedColors().size() > 1) {
                ...
                tooltip
                    .add(
                        Component
                            .translatable(
                                "pastel.tooltip.ink_powered.consume",
                                getUsedColors().get(0).getColoredInkName()
                            )
        ...
        if (stack.getItem() instanceof InkStorageItem<?> inkStorageItem) {
            ...
            if (drained > 0) {
                if (player instanceof ServerPlayer serverPlayerEntity) {
                    PastelCriteria.INK_CONTAINER_INTERACTION
                        .trigger(serverPlayerEntity, stack, inkStorage, color, -amount);
                }
    ...
    static boolean tryDrainEnergy(@NotNull Player player, @NotNull InkCost inkCost, float costModifier) {
        return tryDrainEnergy(
            player,
            inkCost.color(),
            Support.chanceRound(inkCost.cost() * costModifier, player.getRandom())
        );
    }
    ...
    static boolean tryDrainEnergy(@NotNull Player player, @NotNull InkColor color, long amount) {
        ...
        for (
            ItemStack trinketEnergyStorageStack : trinketInkStorages
        ) {
        ...
        // inventory
        for (
            ItemStack itemStack : player.getInventory().items
        ) {
            amount -= tryDrainEnergy(itemStack, color, amount, player);
            if (amount <= 0) {
                return true;
