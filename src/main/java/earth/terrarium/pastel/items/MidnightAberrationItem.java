...
public class MidnightAberrationItem extends CloakedItem {
    ...
    private static final ResourceLocation MIDNIGHT_ABERRATION_CRUMBLING_ADVANCEMENT_ID = PastelCommon.locate(
        "midgame/crumble_midnight_aberration");
    private static final String MIDNIGHT_ABERRATION_CRUMBLING_ADVANCEMENT_CRITERION
        = "have_midnight_aberration_crumble";
    ...
    public MidnightAberrationItem(
        Item.Properties settings, ResourceLocation cloakAdvancementIdentifier, Item cloakItem) {
    ...
    public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
        ...
        if (!world.isClientSide && world.getGameTime() % 20 == 0 && entity instanceof ServerPlayer player) {
            ...
            if (world.random.nextFloat() < 0.2F) {
                stack.shrink(1);
                player.getInventory()
                      .placeItemBackInInventory(Items.GUNPOWDER.getDefaultInstance());
                world.playSound(
                    null, player, PastelSoundEvents.MIDNIGHT_ABERRATION_CRUMBLING, SoundSource.PLAYERS, 0.5F, 1.0F);

                Support.grantAdvancementCriterion(
                    player, MIDNIGHT_ABERRATION_CRUMBLING_ADVANCEMENT_ID,
