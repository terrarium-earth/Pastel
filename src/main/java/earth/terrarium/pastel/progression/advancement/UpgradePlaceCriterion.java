...
public class UpgradePlaceCriterion extends SimpleCriterionTrigger<UpgradePlaceCriterion.Conditions> {
    ...
    public void trigger(
        ServerPlayer player, ServerLevel world, BlockPos pos, int upgradeCount,
    ...
    public record Conditions(
        ...
        public static final Codec<Conditions> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                                                                                                        ...
                                                                                                        MinMaxBounds.Ints.CODEC.optionalFieldOf("count")
                                                                                                                               .forGetter(Conditions::countRange),
                                                                                                        MinMaxBounds.Ints.CODEC.optionalFieldOf("speed_mod")
                                                                                                                               .forGetter(Conditions::speedRange),
                                                                                                        MinMaxBounds.Ints.CODEC.optionalFieldOf("experience_mod")
                                                                                                                               .forGetter(Conditions::experienceRange),
                                                                                                        MinMaxBounds.Ints.CODEC.optionalFieldOf("efficiency_mod")
                                                                                                                               .forGetter(Conditions::efficiencyRange),
                                                                                                        MinMaxBounds.Ints.CODEC.optionalFieldOf("yield_mod")
        ...
        public boolean matches(
            ServerLevel world, BlockPos pos, int upgradeCount, Map<Upgradeable.UpgradeType, Integer> upgradeModifiers) {
            return (this.blockPredicate.isEmpty() || this.blockPredicate.get()
                                                                        .matches(world, pos))
                   && (this.countRange.isEmpty() || this.countRange.get()
                                                                   .matches(upgradeCount))
                   && (this.speedRange.isEmpty() || this.speedRange.get()
                                                                   .matches(upgradeModifiers.get(
                                                                       Upgradeable.UpgradeType.SPEED)))
                   && (this.experienceRange.isEmpty() || this.experienceRange.get()
                                                                             .matches(upgradeModifiers.get(
                                                                                 Upgradeable.UpgradeType.EXPERIENCE)))
                   && (this.efficiencyRange.isEmpty() || this.efficiencyRange.get()
                                                                             .matches(upgradeModifiers.get(
                                                                                 Upgradeable.UpgradeType.EFFICIENCY)))
                   && (this.yieldRange.isEmpty() || this.yieldRange.get()
                                                                   .matches(upgradeModifiers.get(
