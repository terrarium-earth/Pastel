...
public class EggLayingWoolyPigWoolFeatureRenderer
    ...
    @Override
    public void render(
        PoseStack poseStack, MultiBufferSource vertexConsumerProvider, int i, EggLayingWoolyPigEntity entity, float f,
        ...
        if (entity.isInvisible()) {
            ...
            if (hasOutline) {
                ...
                VertexConsumer vertexConsumer = vertexConsumerProvider.getBuffer(
                    RenderType.outline(EggLayingWoolyPigEntityRenderer.TEXTURE));
                if (!entity.isHatless()) {
                    this.getParentModel()
                        .copyPropertiesTo(this.hat);
                    this.hat.prepareMobModel(entity, f, g, h);
                    this.hat.setupAnim(entity, f, g, j, k, l);
                    this.hat.renderToBuffer(
                        poseStack, vertexConsumer, i, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), rgbColor);
                }
                if (!entity.isSheared()) {
                    this.getParentModel()
                        .copyPropertiesTo(this.wool);
                    this.wool.prepareMobModel(entity, f, g, h);
                    this.wool.setupAnim(entity, f, g, j, k, l);
                    this.wool.renderToBuffer(
                        poseStack, vertexConsumer, i, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), rgbColor);
                }
            }
        } else {
            ...
            if (!entity.isHatless()) {
                ...
                coloredCutoutModelCopyLayerRender(
                    this.getParentModel(), this.hat, getTextureLocation(entity), poseStack, vertexConsumerProvider, i,
                    entity, f, g, j, k, l, h, rgbColor
                );
            }
            if (!entity.isSheared()) {
                coloredCutoutModelCopyLayerRender(
                    this.getParentModel(), this.wool, getTextureLocation(entity), poseStack, vertexConsumerProvider, i,
                    entity, f, g, j, k, l, h, rgbColor
                );
            }
        }
