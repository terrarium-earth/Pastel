...
public class ExtendedBundleItem extends BundleItem {
    ...
    public int getBarWidth(ItemStack stack) {
        ...
        return Math.min(
            1 + Mth.mulAndTruncate(Fraction.getFraction(getStacks(stack).size(), getMaxStacks(stack)), 12), 13);
    }
    ...
    public static Fraction getMaxOccupancy(ItemStack stack) {
        return stack.getOrDefault(PastelDataComponentTypes.EXTENDED_BUNDLE, ExtendedBundleComponent.DEFAULT)
                    .maxOccupancy();
    }
    ...
    public static class ComponentBuilder extends BundleContents.Mutable {
        ...
        protected int getMaxAmountToAdd(ItemStack stack) {
            ...
            var allowedByOccupancy = Math.max(
                remainingOccupancy.divideBy(itemOccupancy)
                                  .intValue(), 0
            );
