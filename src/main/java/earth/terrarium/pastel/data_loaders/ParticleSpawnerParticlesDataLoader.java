...
public class ParticleSpawnerParticlesDataLoader extends SimpleJsonResourceReloadListener {
     ...
     * @param supportsColoring  Weather the Particle Spawner enables CMY coloring for this particle (should be true, if
     *                          grayscale)
    ...
    protected void apply(
        ...
        prepared.forEach((identifier, jsonElement) -> {
            ...
            ParticleType<?> particleType = BuiltInRegistries.PARTICLE_TYPE
                .get(ResourceLocation.tryParse(particleTypeString));
            ...
            @Nullable ResourceLocation unlockIdentifier = jsonObject.has("unlock_identifier")
                ? ResourceLocation.tryParse(jsonObject.get("unlock_identifier").getAsString())
                : null;
            boolean supportsColoring = GsonHelper.getAsBoolean(jsonObject, "supports_coloring", false);

            if (particleType == null) {
                PastelCommon
                    .logError("Particle Spawner Particle '" + particleTypeString + "' not found. Will be ignored.");
                return;
            }
    ...
    public static List<ParticleSpawnerEntry> getAllUnlocked(Player player) {
        ...
        for (
            ParticleSpawnerParticlesDataLoader.ParticleSpawnerEntry entry : PARTICLES
        ) {
