...
public abstract class PedestalRecipe extends GatedStackPastelRecipe<PedestalRecipeInput> {
    ...
    public PedestalRecipe(
        ...
        PedestalRecipeTier tier, List<IngredientStack> inputs, Map<GemstoneColor, Integer> powderInputs,
        ItemStack output,
    ...
    public void consumeIngredients(PedestalBlockEntity pedestal) {
        ...
        for (GemstoneColor gemstoneColor : BuiltinGemstoneColor.values()) {
            double efficiencyModifier = pedestal.getUpgradeHolder()
                                                .getEffectiveValue(Upgradeable.UpgradeType.EFFICIENCY);
            int gemstonePowderAmount = this.getGemstonePowderAmount(gemstoneColor);
            int gemstonePowderAmountAfterMod = Support.chanceRound(
                gemstonePowderAmount / efficiencyModifier, world.random);
            pedestal.getItem(PedestalBlockEntity.getSlotForGemstonePowder(gemstoneColor))
                    .shrink(gemstonePowderAmountAfterMod);
        }
    }
    ...
    protected void decrementGridSlot(PedestalBlockEntity pedestal, int slot, int count, ItemStack invStack) {
        ...
        if (remainder.isEmpty()) {
            ...
            if (pedestal.getItem(slot)
                        .getCount() == count) {
                pedestal.setItem(slot, remainder);
            } else {
                pedestal.getItem(slot)
                        .shrink(count);
    ...
    public boolean canCraft(PedestalBlockEntity pedestalBlockEntity) {
        ...
        if (newPedestalVariant == null) {
            return pedestalBlockEntity.getHighestAvailableRecipeTier()
                                      .ordinal() >= this.tier.ordinal();
        } else {
            // pedestal upgrade recipes do not require the structure,
            // but you can only craft it using the previous variant
            int currentTier = PedestalBlockEntity.getVariant(pedestalBlockEntity)
                                                 .getRecipeTier()
                                                 .ordinal();
            int thisTier = newPedestalVariant.getRecipeTier()
                                             .ordinal();
    ...
    public boolean canPlayerCraft(Player playerEntity) {
        return this.tier.hasUnlocked(playerEntity) && AdvancementHelper.hasAdvancement(
            playerEntity, this.requiredAdvancementIdentifier.orElse(null));
    }
