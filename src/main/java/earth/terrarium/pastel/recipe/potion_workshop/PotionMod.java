...
public record PotionMod(
    ...
    public record PotionFlags(
        ...
        private static final Codec<Tuple<PotionRecipeEffect, Float>> ENTRY_CODEC = RecordCodecBuilder.create(
            i -> i.group(
        ...
        private static final StreamCodec<RegistryFriendlyByteBuf, Tuple<PotionRecipeEffect, Float>> ENTRY_PACKET_CODEC
            = PacketCodecHelper.pair(
            PotionRecipeEffect.STREAM_CODEC,
            ByteBufCodecs.FLOAT
        );

        public static final MapCodec<PotionFlags> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
                                                                                                Codec.BOOL.optionalFieldOf("make_splashing", false)
                                                                                                          .forGetter(PotionFlags::makeSplashing),
                                                                                                Codec.BOOL.optionalFieldOf("make_lingering", false)
                                                                                                          .forGetter(PotionFlags::makeLingering),
                                                                                                Codec.BOOL.optionalFieldOf("no_particles", false)
                                                                                                          .forGetter(PotionFlags::noParticles),
                                                                                                Codec.BOOL.optionalFieldOf("unidentifiable", false)
                                                                                                          .forGetter(PotionFlags::unidentifiable),
                                                                                                Codec.BOOL.optionalFieldOf("make_effects_positive", false)
                                                                                                          .forGetter(PotionFlags::makeEffectsPositive),
                                                                                                Codec.BOOL.optionalFieldOf("potent_decreasing_effect", false)
                                                                                                          .forGetter(PotionFlags::potentDecreasingEffect),
                                                                                                Codec.BOOL.optionalFieldOf("negate_decreasing_duration", false)
                                                                                                          .forGetter(PotionFlags::negateDecreasingDuration),
                                                                                                Codec.BOOL.optionalFieldOf("random_color", false)
    ...
    public int getColor(RandomSource random) {
        return flags.randomColor ? java.awt.Color.getHSBColor(random.nextFloat(), 0.7F, 0.9F)
                                                 .getRGB() : flags.unidentifiable ? 0x2f2f2f : -1; // dark gray
    }
    ...
    public static class Builder {
        ...
        public Builder combine(PotionMod potionMod) {
            ...
            this.flags = new PotionFlags(
                ...
                Stream.concat(this.flags.additionalEffects.stream(), potionMod.flags.additionalEffects.stream())
                      .toList()
            );
            return this;
        }
