...
public class ParticleSpawnerParticlesDataLoader extends SimpleJsonResourceReloadListener {
     ...
     * @param supportsColoring  Weather the Particle Spawner enables CMY coloring for this particle (should be true,
     *                          if grayscale)
     * @param unlockIdentifier  The advancement identifier required to being able to select this entry
     */
    public record ParticleSpawnerEntry(
        ParticleType<?> particleType, ResourceLocation textureIdentifier, boolean supportsColoring,
        @Nullable ResourceLocation unlockIdentifier
    ) {
    }
    ...
    protected void apply(
        ...
        prepared.forEach((identifier, jsonElement) -> {
            ...
            String particleTypeString = jsonObject.get("particle_type")
                                                  .getAsString();
            ParticleType<?> particleType = BuiltInRegistries.PARTICLE_TYPE.get(
                ResourceLocation.tryParse(particleTypeString));
            ResourceLocation guiTexture = ResourceLocation.tryParse(jsonObject.get("gui_texture")
                                                                              .getAsString());
            @Nullable ResourceLocation unlockIdentifier = jsonObject.has("unlock_identifier")
                                                          ? ResourceLocation.tryParse(
                jsonObject.get("unlock_identifier")
                          .getAsString()) : null;
            boolean supportsColoring = GsonHelper.getAsBoolean(jsonObject, "supports_coloring", false);

            if (particleType == null) {
                PastelCommon.logError(
                    "Particle Spawner Particle '" + particleTypeString + "' not found. Will be ignored.");
                return;
            }
