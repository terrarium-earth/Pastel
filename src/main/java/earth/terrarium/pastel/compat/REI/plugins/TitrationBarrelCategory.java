...
public class TitrationBarrelCategory extends GatedDisplayCategory<TitrationBarrelDisplay> {
    ...
    public void setupWidgets(
        Point startPoint, Rectangle bounds, List<Widget> widgets, @NotNull TitrationBarrelDisplay display) {
        ...
        if (display.tappingIngredient.isEmpty()) {
            widgets.add(Widgets.createArrow(new Point(startPoint.x + 60, startPoint.y + 11))
                               .animationDurationTicks(display.minFermentationTimeHours * 20));
        } else {
            widgets.add(Widgets.createArrow(new Point(startPoint.x + 60, startPoint.y + 3))
                               .animationDurationTicks(display.minFermentationTimeHours * 20));
            widgets.add(Widgets.createSlot(new Point(startPoint.x + 64, startPoint.y + 21))
                               .markInput()
                               .entries(display.tappingIngredient));
        }
        widgets.add(Widgets.createResultSlotBackground(new Point(startPoint.x + 95, startPoint.y + 11)));
        widgets.add(Widgets.createSlot(new Point(startPoint.x + 95, startPoint.y + 11))
                           .markOutput()
                           .disableBackground()
        ...
        // duration text
        MutableComponent text = TitrationBarrelRecipe.getDurationText(
            display.minFermentationTimeHours, display.fermentationData);
        widgets.add(Widgets.createLabel(new Point(startPoint.x - 10, startPoint.y + 42), text)
                           .leftAligned()
                           .color(0x3f3f3f)
                           .noShadow());
    }
