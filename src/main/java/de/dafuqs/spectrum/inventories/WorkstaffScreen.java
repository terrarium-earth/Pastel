...
public class WorkstaffScreen extends QuickNavigationGridScreen<WorkstaffScreenHandler> {
	...
	private static final Grid RANGE_GRID = new Grid(
			GridEntry.EMPTY,
			GridEntry.text(Component.literal("1x1"), Component.translatable("item.pastel.workstaff.gui.1x1"), (screen) -> WorkstaffScreen.select(WorkstaffItem.GUIToggle.SELECT_1x1)),
			GridEntry.text(Component.literal("5x5"), Component.translatable("item.pastel.workstaff.gui.5x5"), (screen) -> WorkstaffScreen.select(WorkstaffItem.GUIToggle.SELECT_5x5)),
			GridEntry.BACK,
			GridEntry.text(Component.literal("3x3"), Component.translatable("item.pastel.workstaff.gui.3x3"), (screen) -> WorkstaffScreen.select(WorkstaffItem.GUIToggle.SELECT_3x3))
	);
	...
	public WorkstaffScreen(WorkstaffScreenHandler handler, Inventory playerInventory, Component title) {
		...
		if (mainHandStack.getItem() instanceof WorkstaffItem workstaffItem && workstaffItem.canTill(mainHandStack)) {
			...
			rightClickGridEntry = GridEntry.item(SpectrumItems.MULTITOOL, Component.translatable("item.pastel.workstaff.gui.enable_right_click_actions"), (screen) -> WorkstaffScreen.select(WorkstaffItem.GUIToggle.ENABLE_RIGHT_CLICK_ACTIONS));
		}
		...
		if (mainHandStack.getItem() instanceof GlassCrestWorkstaffItem) {
			GridEntry projectileEntry = GlassCrestWorkstaffItem.canShoot(mainHandStack)
					? GridEntry.item(Items.SPECTRAL_ARROW, Component.translatable("item.pastel.workstaff.gui.disable_projectiles"), (screen) -> WorkstaffScreen.select(WorkstaffItem.GUIToggle.DISABLE_PROJECTILES))
					: GridEntry.item(Items.ARROW, Component.translatable("item.pastel.workstaff.gui.enable_projectiles"), (screen) -> WorkstaffScreen.select(WorkstaffItem.GUIToggle.ENABLE_PROJECTILES));
			
			gridStack.push(new Grid(
					GridEntry.CLOSE,
					GridEntry.item(Items.STONE, Component.translatable("item.pastel.workstaff.gui.range_group"), (screen) -> selectGrid(RANGE_GRID)),
					rightClickGridEntry,
					projectileEntry,
					GridEntry.item(Items.ENCHANTED_BOOK, Component.translatable("item.pastel.workstaff.gui.enchantment_group"), (screen) -> screen.selectGrid(ENCHANTMENT_GRID))
			));
		} else {
			...
			gridStack.push(new Grid(
					GridEntry.CLOSE,
					GridEntry.item(Items.STONE, Component.translatable("item.pastel.workstaff.gui.range_group"), (screen) -> screen.selectGrid(RANGE_GRID)),
					rightClickGridEntry,
					GridEntry.EMPTY,
					enchantmentEntry
			));
