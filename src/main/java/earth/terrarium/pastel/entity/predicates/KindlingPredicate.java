...
public record KindlingPredicate(Optional<Boolean> clipped, Optional<Boolean> angry, Optional<KindlingVariant> variant)
    ...
    public static final MapCodec<KindlingPredicate> CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(
                                                                                                                  Codec.BOOL.optionalFieldOf("clipped")
                                                                                                                            .forGetter(KindlingPredicate::clipped),
                                                                                                                  Codec.BOOL.optionalFieldOf("angry")
                                                                                                                            .forGetter(KindlingPredicate::angry),
                                                                                                                  KindlingVariant.CODEC.optionalFieldOf("variant")
        ...
        if (!(entity instanceof KindlingEntity kindling)) {
            ...
            return (this.clipped.isEmpty() || this.clipped.get() == kindling.isClipped())
                   && (this.angry.isEmpty() || this.angry.get() == (kindling.getRemainingPersistentAngerTime() == 0)
                                               && (this.variant.isEmpty() ||
                                                   this.variant.get() == kindling.getKindlingVariant()));
        }
    }
